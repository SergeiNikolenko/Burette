...
This is Burette's internal acceptance checklist, not a claim that every video
or screenshot below is a mandatory OpenAI portal field. The current
[submission guide](https://developers.openai.com/plugins/deploy/submission)
requires accurate listing/policy material, production MCP metadata and five
positive plus three negative cases. Keep the user's one-command, no-attachment
demo separate from this complete operator test matrix.

## Latest engineering checkpoint — 2026-09-13

- Fresh-main integration: `origin/main` at `39466efd44c5947ac433b1d8869da8ca8a2af7ae`
  was merged with remediation head `b4e1a4473` in an isolated worktree.
  Handwritten conflicts preserve the latest Ketcher text panel and shared UI
  plugin together with the continuation/CAS and CSP restrictions. Agent-shell
  artifacts were rebuilt from that source instead of merging generated files.
- Integrated source: agent-shell and hosted production builds passed on Gauss;
  hosted TypeScript and all 94 hosted tests passed (including the incoming
  drawing-preservation regression adapted to continuation tokens). Focused
  shell, split-globals startup, Vite plugin, Ketcher primitives and sequence
  tests passed. These are build/protocol checks, not native-host acceptance.
- Independent merge review found a missing hosted zoom control after main's
  toolbar refactor; the same scale-control group now serves both headers.
  Real strict-CSP browser testing also caught Paper's bundled PaperScript
  parser executing dynamic code. Hosted builds now select the upstream
...
- Hosted candidate: production assets and Next.js build passed on Gauss;
  package TypeScript check and all 93 hosted tests passed. This is a candidate
  build, not a deployment or ChatGPT/iPhone acceptance result.
- Desktop hosted startup handles separately delivered result/metadata; focused
  integration checks passed without requiring camera reset in the test harness.
  Actual first-paint behavior in ChatGPT still requires the fresh-host check.
- Widget bridge now configures Zod's interpreted validator before MCP schema
  initialization. Its bundled regression test detects attempted dynamic code
  generation. CSP was not weakened.
- Local plugin `0.2.2+local.20260913.6` was installed separately. Its compact
  and native protocol fixtures loaded molecules with no collected CSP/runtime
  errors; pane/inline transitions preserved the renderer. These fixtures are
  explicitly not native Codex or hosted ChatGPT acceptance.
- Live read-only preflight: health, five tools and their annotations/schemas,
  two resource URIs, viewer-shell security headers and four public policy/docs
  URLs responded successfully. No deployment identity, portal rescan, production
  Redis settings or physical-device acceptance was verified in this checkpoint.
- Publication blocker: the live July 10 privacy policy does not yet describe
  the hosted Ketcher continuation/CAS state or the widget pageview. Update and
  approve the public policy before submission; source-only copy is insufficient.
- No production deployment, connector rescan or OpenAI submission was performed.
...
- [ ] Collapse and expand the result, then reload the ChatGPT conversation and remount the latest Ketcher result. Confirm the latest benzene structure and atom-5 highlight, surface identity, and current revisions rehydrate without a blank editor or `STALE_TARGET`; do not expect the earlier acetic-acid seed to return.

...
function boundedIdentity(value: unknown) {
  ...
  return {
    ...
    ...(typeof atom.model === "string" ? { model: bounded(atom.model) } : {}),
    ...(typeof atom.unit === "number" ? { unit: atom.unit } : {}),
    ...(typeof atom.operator === "string" ? { operator: bounded(atom.operator) } : {}),
    ...(typeof atom.atomIndex === "number" ? { atomIndex: atom.atomIndex } : {}),
  };
}
...
export function createSelectionContext(value: unknown, documentId: string, source?: unknown) {
  ...
  const documentLabel = sourceDescriptor?.kind === "attachment"
    ? sourceDescriptor.fileName : sourceDescriptor?.kind === "pdb" ? sourceDescriptor.pdbId : "Active structure";
  if (!selection || selection.cleared === true) {
    return {
      content: [{ type: "text" as const, text: `Burette · ${documentLabel}. No active selection; context refers to the open structure.` }],
      structuredContent: { burette: { source: sourceDescriptor, activeSelection: null } },
    };
  }
  const atomIdentities = Array.isArray(selection.atomIdentities)
    ...
    : [];
  const residues = Array.isArray(selection.residues) && selection.residues.length
    ? selection.residues.slice(0, MAX_ITEMS).map(boundedIdentity).map(({ atomName: _, ...residue }) => residue)
    : Array.from(new Map(atomIdentities.map(({ atomName: _, atomIndex: __, ...residue }) => [JSON.stringify(residue), residue])).values());
  const atoms = Math.max(0, Math.trunc(Number(selection.atoms) || atomIdentities.length));
  const residueCount = Math.max(0, Math.trunc(Number(selection.residueCount) || residues.length));
  const residueLabels = residues.slice(0, 12).map(residue =>
    `${residue.compId} ${residue.chain} ${residue.sequence ?? ""}`.trim()).join(", ");
  const label = bounded(selection.label, `Selection with ${atoms} visible atoms across ${residues.length} residues`);
  const context = {
    content: [{
      type: "text" as const,
      text: `Burette · ${documentLabel}. Selected: ${residueLabels || label}. ${atoms} atoms; ${selection.truncated ? "at least " : ""}${residueCount} residues${residues.length > 12 ? " (first 12 listed)" : ""}.${selection.level ? ` Picking level: ${bounded(selection.level)}.` : ""}${selection.truncated ? " Atom identities are a bounded sample." : ""}`,
    }],
    structuredContent: {
      burette: {
        ...
        activeSelection: {
          ...
          residues,
          atomIdentities,
          ...(selection.selector ? { selector: sanitizedSelector(selection.selector) } : {}),
          ...(typeof selection.level === "string" ? { level: bounded(selection.level) } : {}),
          ...(typeof selection.truncated === "boolean" ? { truncated: selection.truncated } : {}),
        },
      },
    },
  };
  const active = context.structuredContent.burette.activeSelection;
  const encoder = new TextEncoder();
  while (encoder.encode(JSON.stringify(context)).byteLength > 24 * 1024) {
    if (active.atomIdentities.length) active.atomIdentities.pop();
    else if (active.residues.length) active.residues.pop();
    else break;
...
export function createSceneContext(value: unknown, source?: unknown) {
  ...
  return {
    content: [{
      type: "text" as const,
      text: `Burette applied ${results.filter((result) => result.ok).length} of ${results.length} requested viewer actions.`,
    }],
    structuredContent: {
      burette: {
        source: boundedSource(source),
        activeSelection: selectionContext.structuredContent.burette.activeSelection,
        scene: {

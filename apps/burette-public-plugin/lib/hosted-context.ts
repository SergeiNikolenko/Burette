...
export function createSelectionContext(value: unknown, documentId: string, source?: unknown) {
  ...
  if (!selection || selection.cleared === true) {
    return {
      content: [{ type: "text" as const, text: "The active Burette molecular selection is empty." }],
  ...
  return {
    content: [{
      type: "text" as const,
      text: `Current Burette selection: ${label}. Treat this as the user's active molecular selection for their next request.`,
    }],
    structuredContent: {
      burette: {

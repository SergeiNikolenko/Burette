...
describe("viewer resource contract", () => {
  test("allows only the hosted Burette runtime and same-origin assets", () => {
    ...
    expect(meta.ui.csp.resourceDomains).toEqual(["https://burette.example"]);
  ...
  test("mounts the real Burette shell directly and listens for MCP tool results", () => {
    const html = createViewerWidgetHtml("https://burette.example");
    ...
    expect(html).toContain("__BURETTE_HOSTED_MCP_WIDGET__");
    expect(html).toContain(
      '"analyticsOrigin":"https://burette.example"',
    );
    expect(html).toContain("__BURETTE_HOSTED_ANALYTICS_ORIGIN__");
  ...
  test("uses a direct mobile viewer without a second iframe", () => {
    const source = readFileSync(path.resolve(
      import.meta.dir,
      "../assets/burette-hosted-mobile.js",
    ), "utf8");
    expect(source).toContain('root.id = "app"');
    expect(source).toContain('quickLookBuild: "burette-hosted-mobile-direct"');
    ...
    expect(source).toContain('window.BuretteMolstarURL = `${assetsBase}/molstar.js`');
    ...
    expect(source).toContain("BuretteHostedAppBridge");
  ...
  test("bootstraps inside a WKWebView with protected native WebKit globals", () => {
    ...
    const window = { parent } as {
      ...
      webkit?: unknown;
      BuretteConfig?: Record<string, unknown>;
    ...
    const runtimeElements = new Map([
      ["burette-runtime-config", { textContent: JSON.stringify({ documentId: "document-1" }) }],
      ["burette-runtime-data", { textContent: JSON.stringify("QUJD") }],
    ]);
    const document = {
    ...
    expect(window.BuretteConfig).toEqual({ documentId: "document-1" });
    expect(window.BuretteDataBase64).toBe("QUJD");
  ...
  test("preserves the native Quick Look message handler", () => {
    ...
    const window = {
      ...
      webkit: Object.freeze({
        messageHandlers: Object.freeze({ burette: nativeHandler }),
      }),
    } as {
      parent: null;
      webkit: unknown;
  ...
  test("does not expose local diagnostics inside the hosted widget", () => {
    ...
    expect(source).toContain("window.__BURETTE_HOSTED_MCP_WIDGET__ === true");
  ...
  test("initializes the Apps bridge before publishing bounded viewer state", () => {
    ...
    expect(selection.structuredContent.burette.activeSelection?.atomIdentities).toEqual([
      { chain: "A", sequence: 12, compId: "CYS", atomName: "CA" },
    ]);
    expect(selection.structuredContent.burette.source).toEqual({ kind: "pdb", pdbId: "1CRN" });
    expect(createSelectionContext(null, "document-1", {
      kind: "attachment",
      fileName: "example.pdb",
      nested: { unbounded: "x".repeat(10_000) },
    }).structuredContent.burette.source).toEqual({
      kind: "attachment",
      fileName: "example.pdb",
    });
    expect(createSelectionContext(null, "document-1").structuredContent.burette.activeSelection).toBeNull();
    const picked = createSelectionContext({
      atoms: 2,
      atomIdentities: [
        { chain: "A", sequence: 377, compId: "NAD", atomName: "C1" },
        { chain: "A", sequence: 377, compId: "NAD", atomName: "N1" },
      ],
    }, "document-1", { kind: "attachment", fileName: "1htb.pdb" });
    expect(picked.structuredContent.burette.activeSelection?.residues).toEqual([
      { chain: "A", sequence: 377, compId: "NAD" },
    ]);
    expect(picked.structuredContent.burette.activeSelection?.residueCount).toBe(1);
    expect(picked.content[0].text).toContain("1htb.pdb");
    expect(picked.content[0].text).toContain("NAD A 377");
    expect(picked.content[0].text).toContain("2 atoms");
    expect(createSelectionContext(null, "document-1", {
      kind: "attachment", fileName: "1htb.pdb",
    }).content[0].text).toContain("1htb.pdb");

    expect(sanitizeViewerActions([
      { type: "hide_components", kind: "water" },
      { type: "raw_burette_agent", command: "loadMVS" },
    ...
    expect(scene.structuredContent.burette.scene.results).toEqual([
  ...
  test("bootstraps hosted viewer data without executable inline scripts", () => {
    ...
    expect(browserDocumentsSource).toContain('id="burette-runtime-config" type="application/json"');
    expect(browserDocumentsSource).toContain('id="burette-runtime-data" type="application/json"');
  ...
  test("hardens the directly served shell and enables cross-origin assets", async () => {
    ...
    expect(webDemoDocument?.headers).toContainEqual({
      key: "Content-Security-Policy",
      value: expect.stringContaining("frame-ancestors 'self' https://burette-landing.vercel.app"),
    });
  });

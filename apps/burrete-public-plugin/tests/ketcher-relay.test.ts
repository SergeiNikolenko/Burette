...
import { KETCHER_AGENT_API_VERSION } from "@burrete/ketcher-agent-contract";
...
describe("hosted Ketcher widget contract", () => {
  test("uses the versioned resource and same-origin CSP", () => {
    const meta = createKetcherResourceMeta("https://burrete.example");
    const toolMeta = ketcherToolMeta("Opening Ketcher…", "Ketcher ready");
    expect(KETCHER_RESOURCE_URI).toBe("ui://burrete/ketcher-editor-v1.html");
  ...
  test("bootstraps the hosted seed bridge without widening the payload path", () => {
    const html = createKetcherWidgetHtml("https://burrete.example");
    expect(html).toContain("__BURRETE_HOSTED_KETCHER_WIDGET__");
    expect(html).toContain("__BURRETE_HOSTED_KETCHER_SEED__");
    expect(html).toContain("burrete-ketcher-seed");
    expect(html).toContain("callServerTool");
    expect(html).toContain("/viewer-shell/assets/burrete-hosted-shell.js");

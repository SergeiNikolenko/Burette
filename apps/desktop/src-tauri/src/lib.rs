...
use tauri::{DragDropEvent, Emitter, Manager, RunEvent, Runtime, WebviewWindow, WindowEvent};

use molecule::formats::{
    format_for_extension, normalize_renderer_mode, resolve_renderer, FormatInfo,
};
use path_utils::{escape_html, file_title, stable_id};

const MAX_STRUCTURE_FILE_SIZE: u64 = 75 * 1024 * 1024;

fn queue_open_payload(app: &tauri::AppHandle, label: &str, payload: open_target::PendingOpenPayload) {
...
fn route_first_open_payload(app: &tauri::AppHandle, label: &str, paths: Vec<PathBuf>) -> bool {
    let Some(payload) = open_target::open_payloads_from_paths(paths).into_iter().next() else {
        return false;
    };

    let should_open_new_window = app
        ...
        .is_some_and(|root| root != PathBuf::from(&payload.workspace));

    if should_open_new_window {
        if open_new_workspace_window(app, payload.workspace.clone(), payload.file.clone()).is_ok() {
            return true;
        }
    }

    queue_open_payload(app, label, payload);
    true
}
...
fn attach_window_handlers(app: &tauri::AppHandle, window: &WebviewWindow) {
    ...
    window.on_window_event(move |event| match event {
        WindowEvent::DragDrop(DragDropEvent::Drop { paths, .. }) => {
            let _ = route_first_open_payload(&handle, &label, paths.clone());
        }
        WindowEvent::Destroyed => {
            let _ = handle.state::<state::AppState>().remove(&label);
        }
        _ => {}
    });
}
...
pub(crate) fn open_new_workspace_window(
    ...
    let workspace = PathBuf::from(&workspace_path)
        .canonicalize()
        .map_err(|err| format!("{}: {err}", workspace_path))?;
    if !workspace.is_dir() {
        return Err(format!("{} is not a folder", workspace.display()));
    }

    if let Some(existing_label) = app.state::<state::AppState>().find_by_workspace(&workspace) {
    ...
    let state = app.state::<state::AppState>().get_or_create(&label);
    state.push_pending_open(open_target::PendingOpenPayload {
        workspace: workspace.to_string_lossy().to_string(),
        file,
    });

    let Some(mut window_config) = app.config().app.windows.first().cloned() else {
        app.state::<state::AppState>().remove(&label);
        return Err("No window config in tauri.conf.json".to_string());
    };
...
pub(crate) fn is_supported_structure_path(path: &Path) -> bool {
    ...
    format_for_extension(&extension).is_ok() || grid_can_preview(&extension)
}
...
pub(crate) fn modified_seconds(metadata: &fs::Metadata) -> u64 {
    metadata
        .modified()
        .ok()
...
fn bundled_web_dir<R: Runtime>(app: &tauri::AppHandle<R>) -> Result<PathBuf, String> {
    ...
    for root in manifest_dir.ancestors() {
        let candidate = root.join("PreviewExtension").join("Web");
        if candidate.exists() {
            return Ok(candidate);
        }
    }
    ...
    tauri::Builder::default()
        ...
        .plugin(tauri_plugin_single_instance::init(|app, argv, cwd| {
            let paths: Vec<PathBuf> = argv
                ...
                .skip(1)
                .map(PathBuf::from)
                .map(|path| if path.is_absolute() { path } else { PathBuf::from(&cwd).join(path) })
                .collect();
            let queued = route_first_open_payload(app, "main", paths);
            if !queued {
                commands::startup::emit_open_documents(app, Vec::new());
            }
        ...
        .setup(|app| {
            app_menu::configure_menu(app)?;
            let app_handle = app.handle().clone();
            app_menu::install_menu_event_handler(app);
            #[cfg(target_os = "macos")]
            if let Some(window) = app_handle.get_webview_window("main") {
                attach_window_handlers(&app_handle, &window);
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::startup::startup_open_payload,
            commands::documents::open_documents,
            commands::fs_actions::read_directory,
            commands::search::search_workspace,
            commands::fs_actions::file_exists,
            commands::fs_actions::create_empty_file,
            commands::fs_actions::create_directory,
            commands::fs_actions::rename_entry,
            commands::fs_actions::duplicate_entry,
            commands::fs_actions::delete_entry,
            commands::fs_actions::clear_preview_cache,
            commands::fs_actions::open_logs_folder,
            commands::fs_actions::reveal_path,
            commands::fs_actions::open_external_url,
            commands::settings::get_settings,
            commands::settings::get_setting,
            commands::settings::set_setting,
            commands::settings::reset_setting,
            commands::window::development_instance_name,
            commands::window::titlebar_double_click_action,
            commands::workspace_store::get_recent_workspaces,
            commands::workspace_store::open_workspace,
            commands::workspace_store::restore_workspace,
            commands::workspace_store::take_pending_open,
            commands::workspace_store::resolve_open_payload,
            commands::workspace_store::open_workspace_in_new_window,
            commands::workspace_store::remember_workspace,
            commands::workspace_store::remove_recent_workspace,
            commands::workspace_store::save_session,
            commands::workspace_store::load_session,
            #[cfg(target_os = "macos")]
        ...
        .run(|app, event| match event {
            RunEvent::Opened { urls } => {
                let paths: Vec<PathBuf> = urls
                    ...
                    .collect();
                if !route_first_open_payload(app, "main", paths) {
                    commands::startup::emit_open_documents(app, Vec::new());
                }
            }
            RunEvent::WindowEvent {
                ...
                let _ = app.state::<state::AppState>().remove(&label);
            }
            _ => {}
        });
}
...
mod tests {
    ...
    #[test]
    fn read_directory_returns_supported_entries_with_folders_first() {
        let root = temp_workspace("read-directory");
        let nested = root.join("nested");
        let empty = root.join("empty");
        fs::create_dir_all(&nested).expect("create nested dir");
        fs::create_dir_all(&empty).expect("create empty dir");
        fs::write(root.join("zeta.pdb"), b"HEADER\n").expect("write pdb");
        fs::write(root.join("notes.txt"), b"ignore").expect("write txt");
        fs::write(root.join(".hidden.pdb"), b"ignore").expect("write hidden");
        fs::write(nested.join("alpha.sdf"), b"mol\n").expect("write sdf");

        let entries = commands::fs_actions::read_directory(root.to_string_lossy().to_string())
            .expect("read directory");
        let names: Vec<String> = entries.iter().map(|entry| entry.name.clone()).collect();

        assert_eq!(names, vec!["empty", "nested", "zeta.pdb"]);
        assert!(entries[0].is_directory);
        assert!(entries[1].is_directory);
        assert!(entries[2].is_structure_file);
        assert_eq!(entries[2].byte_count, Some(7));

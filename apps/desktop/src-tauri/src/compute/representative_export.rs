...
use burrete_compute_protocol::{
    ArtifactFile, ArtifactManifest, JobSnapshot, MolecularSnapshotRecordV1,
    OrderedRecordMoleculeIdentityHasher, MOLECULAR_RECORDS_FILE_PATH,
};
use rustix::{
    fs::{fstat, open, openat, renameat_with, FileType, Mode, OFlags, RenameFlags},
    process::geteuid,
};
use serde::Serialize;
...
fn open_verified_artifact_file(
    ...
    let (directory_name, file_name) = relative_path
        ...
        .ok_or_else(|| protocol("artifact export path must contain exactly two components"))?;

...
export function DockPanel({ area, state, actions, readOnly = false }: DockPanelProps) {
  ...
  const catalog = dockTabCatalog(area).filter((kind) => !state.buildInfo.isAgentShell || !["jobs", "chemical-space", "folding"].includes(kind));
  ...
  if (state.buildInfo.isAgentShell && tabs.length === 0) tabs.push(createDockTab(area === "right" ? "inspector" : "files"));
...
function DockPanelContent({
  ...
  if (activeTabKind === "text") {
    if (area === "right" && activePageKind === "ketcher") {
      if (state.buildInfo.isAgentShell) return <KetcherDockTool area={area} state={state} fileTabs={null} />;
      return (
        <DockEmptyState
...
function KetcherDockTool({
  ...
  if (area === "right" && !state.buildInfo.isAgentShell) {
    return (
      <div className="dock-files-view">
        {fileTabs}
  ...
  return (
    <div className="dock-files-view">
      {fileTabs}
      <div className="ketcher-dock-portal" data-ketcher-dock-portal={area}>
        <div className="dock-content dock-content-empty">

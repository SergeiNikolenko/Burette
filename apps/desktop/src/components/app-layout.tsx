...
function rightDockMaxWidth(workbenchWidth: number, fitEditor = false) {
  if (fitEditor) return Math.max(0, Math.floor(workbenchWidth * 0.5));
...
function clampRightDockWidth(width: number, workbenchWidth: number, fitEditor = false) {
  const maxWidth = rightDockMaxWidth(workbenchWidth, fitEditor);
...
export function AppLayout({
  ...
  const nativePlacement = useNativeWorkspacePlacement();
  const bottomDockAvailable = !hostedMcpWidget && nativePlacement?.mode !== "inline";
  ...
  const sidebarVisible = !state.buildInfo.isAgentShell && (settingsMode || (!hostedMcpWidget && state.sidebarOpen));
  ...
  const fitEditor = state.buildInfo.isAgentShell && state.activeTab?.location.kind === "ketcher";
  const mainMinLayoutWidth = fitEditor ? Math.floor(workbenchWidth * 0.5) : Math.max(0, MAIN_MIN_WIDTH - rightDockOverlap(workbenchWidth));
  const rightDockWidth = clampRightDockWidth(state.rightDockWidth, workbenchWidth, fitEditor);
  ...
  const chromeLeadingInset = compactLeadingChrome
    ? state.buildInfo.isAgentShell ? "72px" : "112px"
    : "calc(92px / var(--window-zoom, 1) + 100px)";
  const rightDockOpen = !settingsMode && !hostedMcpWidget && state.rightDockOpen;
  const bottomDockOpen = !settingsMode && bottomDockAvailable && state.bottomDockOpen;
  ...
  return (
    <main
      ...
      data-agent-shell={state.buildInfo.isAgentShell ? "true" : undefined}
        ...
        <>
          {!hostedMcpWidget ? (
            <div className="chrome-leading-controls" data-tauri-drag-region>
              {!state.buildInfo.isAgentShell && <button
                type="button"
        ...
        >
          ...
          >
            <div className="sidebar-shell-inner">
              {!state.buildInfo.isAgentShell && <Sidebar state={layoutState} actions={actions} open={sidebarVisible} />}
            </div>
          </ResizablePanel>
          {chromeVisible && !state.buildInfo.isAgentShell ? (
            ...
            <section className="workbench">
              ...
              >
                <ResizablePanel
                  ...
                  style={fitEditor ? CLIPPED_PANEL_STYLE : SPILLING_PANEL_STYLE}
                >
                  <ResizablePanelGroup
                    orientation="vertical"
                    className="workbench-main-panels"
                    style={{ minWidth: fitEditor ? 0 : MAIN_MIN_WIDTH }}
                    elementRef={workbenchMainGroupRef}
                    data-panels-animating={bottomDockAnimating || undefined}
                    onLayoutChanged={(_layout, meta) => {
                      if (!meta.isUserInteraction || !bottomDockAvailable) return;
                  ...
                  >
                    ...
                    {chromeVisible && bottomDockAvailable ? (
                    ...
                    >
                      ...
                      <DockPanel area="bottom" state={bottomDockAvailable ? layoutState : { ...layoutState, bottomDockOpen: false }} actions={actions} />
                    </ResizablePanel>
                  </ResizablePanelGroup>
                </ResizablePanel>
                {chromeVisible ? (
                ...
                <ResizablePanel
                  ...
                  minSize="260px"
                  maxSize={`${rightDockMaxWidth(workbenchWidth, fitEditor)}px`}
                  groupResizeBehavior="preserve-pixel-size"
                >

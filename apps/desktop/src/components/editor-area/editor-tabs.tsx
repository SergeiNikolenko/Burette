...
import { useTabPointerReorder } from "./use-tab-pointer-reorder";
...
export function EditorTabs({
  ...
  const nativeWorkspace = typeof window !== "undefined" && Boolean(window.BuretteMcpWorkspace);
  ...
  useLayoutEffect(() => {
    ...
    if (nativeWorkspace || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
  ...
  const pointerReorder = useTabPointerReorder(moveDraggedTab, setDraggingTabId);
          ...
          return (
            ...
            >
              <DocumentTab
                ...
                active={active}
                dirty={dirty}
                onClose={readOnly ? undefined : () => actions.closeTab(tab.id)}
                draggable={!readOnly && !nativeWorkspace}
                aria-grabbed={isDragging || undefined}
                onPointerDown={readOnly || !nativeWorkspace ? undefined : event => pointerReorder.start(tab.id, event)}
                onMouseDown={readOnly ? undefined : (event) => {
                  if (event.button === 2) {
                    showTabMenu(event);
                    return;
                  }
                  if (!nativeWorkspace) startMouseTabReorder(tab.id, true, event);
                }}
                onClick={(event) => {
                  if (nativeWorkspace && pointerReorder.consumeClick()) {
                    event.preventDefault();
                    return;
                  }
                  handleTabClick(tab.id, event);
                }}

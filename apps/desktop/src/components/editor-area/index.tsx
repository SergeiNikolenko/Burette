...
export function ViewerArea({ state, actions }: { state: ShellViewState; actions: ShellActions }) {
  ...
  const dirtyTabIds = useMemo(() => new Set(tabs.filter((tab) => (
    (window.BuretteMcpWorkspace && tab.location.kind === "ketcher")
  ...
  return (
    <div className="page-stack">
      {/* Moving an iframe DOM node reloads its browsing context, even with a
          stable React key. Page stacking order must not follow the tab strip. */}
      {[...tabs].sort((left, right) => left.id.localeCompare(right.id)).map((tab) => {
        const isActive = tab.id === tabs[activeTabIndex]?.id;
...
function mountedWarmPageLimit() {
  // The native workspace already bounds admitted files (8 files / 16 MiB).
  // Keep visited pages until close: cycling through documents must not dispose
  // their camera, selection, grid position or editor. Unvisited pages stay cold.

...
export function OpenInEditorMenu({ state, actions, presentation = "chrome" }: { state: ShellViewState; actions: ShellActions; presentation?: "chrome" | "file-header" }) {
  ...
  const nativeIcons = useNativeApplicationIcons(activeFile?.path ?? null, targets);
  ...
  }, [actions, activeFile, defaultApplicationIconUrl, finderIconUrl, loadedPath, loading, targets, nativeIcons]);

  if (!activeFile) return null;

  const visibleTargets = activeFile ? (targets.length > 0 ? targets : browserDevPreviewTargets(activeFile.path)) : [];
...
function activeFileFromState(state: ShellViewState): ActiveFile | null {
  if (window.BuretteMcpWorkspace && state.activeDocument?.virtual) return null;

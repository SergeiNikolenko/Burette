...
export function WelcomeScreen({ actions, buildInfo }: { actions: ShellActions; buildInfo: BuildInfo }) {
  if (buildInfo.isAgentShell) {
    return (
      <Empty className="new-tab-page border-0">
        <EmptyHeader className="max-w-sm">
          <EmptyTitle className="text-xl font-normal">Burette</EmptyTitle>
          <EmptyDescription>Ask in chat to open a structure or a molecule collection.</EmptyDescription>
        </EmptyHeader>
        <EmptyContent>
          <Button variant="outline" onClick={actions.openKetcher}>Draw a molecule</Button>
        </EmptyContent>
      </Empty>
    );
  }

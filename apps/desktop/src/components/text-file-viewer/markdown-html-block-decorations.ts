...
function sanitizeHTML(html: string): string {
  ensureSanitizer();
  return DOMPurify.sanitize(convertMarkdownInHtml(html), {
    ALLOWED_TAGS,
    ALLOWED_ATTR,
...
const htmlBlockTheme = EditorView.baseTheme({
  ".cm-html-block-widget": { padding: "0.25em 0", lineHeight: "1.6" },
  ".cm-html-block-widget h1, .cm-html-block-widget h2, .cm-html-block-widget h3, .cm-html-block-widget h4, .cm-html-block-widget h5, .cm-html-block-widget h6":
    { marginTop: "0.5em", marginBottom: "0.25em", fontWeight: "600", lineHeight: "1.3" },
  ...
  ".cm-html-block-widget a": { color: "var(--link-color, #4fc1ff)", textDecoration: "underline" },
  ".cm-html-block-widget img": { maxWidth: "100%", height: "auto" },
  ".cm-html-block-widget table": { borderCollapse: "collapse", width: "100%" },
  ...
  ".cm-html-block-widget blockquote": {
    borderLeft: "3px solid var(--blockquote-border, var(--line-subtle))",
    paddingLeft: "1em",
    margin: "0.5em 0",
  ...
  ".cm-html-block-widget ul, .cm-html-block-widget ol": { paddingLeft: "1.5em", margin: "0.25em 0" },
  ".cm-html-block-widget details": {
    border: "1px solid var(--border-color, #3e3e42)",
    borderRadius: "4px",
    padding: "0.5em",
  },
  ".cm-html-block-widget summary": { cursor: "pointer", fontWeight: "600" },
});
...
const detailsBlockParser: BlockParser = {
  ...
  parse(cx: BlockContext, line: Line) {
    ...
    while (!endPattern.test(line.text) && cx.nextLine()) {
      // Continue until the closing details tag, matching Writer's block parser.
    }
    cx.nextLine();
    cx.addElement(cx.elt("HTMLBlock", from, cx.prevLineEnd()));
    return true;
...
export function markdownHtmlBlockDecorations() {

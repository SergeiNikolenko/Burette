...
const BURETTE_KETCHER_BUTTONS = {
  ...
  images: { hidden: true },
  analyse: { hidden: defaultBuildInfo.isAgentShell },
...
function readSelectedAtomIndexes(editor: KetcherDirectEditor) {
  return uniqueIndexes(editor.selection()?.atoms ?? []);
}
...
export function KetcherEditor({
  ...
  const instanceRef = useRef<KetcherWithEditorStruct | null>(null);
  ...
  const handleInit = useCallback((instance: KetcherReactInstance) => {
    ...
    instanceRef.current = compatibleInstance as KetcherWithEditorStruct;
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(() => {
      suppress();
      if (!defaultBuildInfo.isAgentShell || !root.clientWidth || !root.clientHeight) return;
      // Host placement changes resize the canvas without recentering Ketcher's
      // drawing. Keep the sketch visible without changing its data or zoom.
      window.cancelAnimationFrame(resizeFrame);
      resizeFrame = window.requestAnimationFrame(() => instanceRef.current?.editor.centerStruct());
    });

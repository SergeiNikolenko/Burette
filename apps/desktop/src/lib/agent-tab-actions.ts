...
export async function executeAgentTabAction(
  ...
  if (operation === "close_all" || operation === "close_others") {
    const closing = tabs.filter(tab => operation === "close_all" || tab.id !== target.id);
    closing.forEach(tab => actions.closeTab(tab.id));
    return success(operation, { requestedTabIds: closing.map(tab => tab.id) });
  }
  if (operation === "move") {
  ...
  return failure("INVALID_ARGS", "manage_tabs operation must be focus, next, previous, open_file, new, close, close_all, close_others, or move.");
}

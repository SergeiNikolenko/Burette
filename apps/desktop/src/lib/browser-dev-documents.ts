...
import { defaultBuildInfo } from "./build-info";
...
const GRID_ASSET_VERSION = "grid-ui-v184";
...
export async function openBrowserDevDockingDocument(
  ...
  const config = {
    ...
    pubChemSearch: true,
    tauriViewer: false,
    visualizationOnly: defaultBuildInfo.isAgentShell,
...
function viewerHtml(
  ...
  if (defaultBuildInfo.isAgentShell) config.visualizationOnly = true;
  config.autoFocusStructure = true;
  if (mesoscale) {

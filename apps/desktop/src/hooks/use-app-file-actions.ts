...
export function useAppFileActions({
  ...
  const listChemicalEditorTargets = useCallback(async (path: string): Promise<ChemicalEditorTarget[]> => {
    if (isBrowserAgentShell) {
      try { return (await browserFileAction({ type: "list_apps", path })).targets ?? []; }
      catch (error) { pushErrorStatus(error, "Application discovery failed"); return []; }
    }
    if (!isTauriRuntime()) {
  ...
  const openPathWithDefaultApp = useCallback(async (path: string) => {
    try {
      if (isBrowserAgentShell) await browserFileAction({ type: "open_default", path });

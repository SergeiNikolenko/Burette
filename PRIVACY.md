...
This policy covers the Burette website, hosted OpenAI molecular viewer and MCP
...
processed on the user's device. They do not upload a file to the hosted Burette
...
Burette with a temporary HTTPS download URL, file identifier, filename, and MIME
type authorized for that tool call. Burette fetches up to 3 MiB, parses the
...
PDB files to Burette application storage. File contents remain in server memory
...
For a PDB lookup, Burette receives the requested four-character PDB ID and sends
...
display its viewer. Vercel hosts the Burette MCP service and website and may
...
disabled. Burette does not include a PDB ID, filename, molecular content,
...
Burette does not intentionally configure application logs to record raw
...
account settings and policies. Burette does not sell personal information or
molecular-file data.

## Network access

Burette may make network requests when the user asks it to retrieve an
...
Burette may store local preferences, recent-file references, logs, preview
...
Users can clear Burette caches and diagnostics from the application settings or
...
not retained in Burette application storage.
...
<https://github.com/SergeiNikolenko/Burette/security/advisories/new>. Use
<https://burette-landing.vercel.app/support> for non-sensitive support.

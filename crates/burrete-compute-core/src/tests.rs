...
use burrete_compute_protocol::SimilarityCutoff;
...
fn options(tile_size: usize, max_undirected_edges: u64) -> GraphBuildOptions {
    GraphBuildOptions {
        tile_size: NonZeroUsize::new(tile_size).expect("nonzero tile"),
...
fn empty_and_zero_fingerprints_follow_explicit_cutoff_semantics() {
    ...
    assert!(butina_clusters(&empty).is_empty());
    ...
    assert_eq!(butina_clusters(&positive), vec![vec![0], vec![1]]);
    ...
    assert_eq!(butina_clusters(&zero), vec![vec![0, 1]]);
}
...
fn butina_ties_choose_the_lowest_live_index() {
    ...
    assert_eq!(butina_clusters(&cycle), vec![vec![0, 1, 3], vec![2]]);
}

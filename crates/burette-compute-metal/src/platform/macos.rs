...
use burette_compute_protocol::{GpuDeviceIdentity, SimilarityCutoff};
use metal::{
...
type Pm6FockPairV1 = Rm1FockPairV1;
...
const PM6_D3_TABLE: &[u8] =
    include_bytes!("../../../burette-compute-core/src/semiempirical/pm6_d3.generated.bin");
...
pub(crate) struct MetalHost {
    ...
    pm6_pair_fock_pipeline: ComputePipelineState,
...
impl MetalHost {
    ...
    fn from_library(device: Device, library: &LibraryRef) -> Result<Self, MetalRuntimeError> {
        let degree_pipeline = pipeline(&device, library, "burette_tanimoto_degree_count_v1")?;
        let fill_pipeline = pipeline(&device, library, "burette_tanimoto_csr_fill_v1")?;
        let query_pipeline = pipeline(&device, library, "burette_tanimoto_query_counts_v1")?;
        let counts_batch_pipeline = pipeline(&device, library, "burette_tanimoto_counts_batch_v1")?;
        let top_k_batch_pipeline = pipeline(&device, library, "burette_tanimoto_top_k_batch_v1")?;
        let umap_initialize_pipeline = pipeline(&device, library, "burette_umap_initialize_v1")?;
        let umap_epoch_pipeline = pipeline(&device, library, "burette_umap_epoch_v1")?;
        let conformer_initialize_pipeline =
            pipeline(&device, library, "burette_conformer_initialize_v1")?;
        let conformer_distance_pipeline =
            pipeline(&device, library, "burette_conformer_distance_v1")?;
        let conformer_optimize_pipeline =
            pipeline(&device, library, "burette_conformer_optimize_v1")?;
        let conformer_stereo_pipeline =
            pipeline(&device, library, "burette_conformer_stereo_validate_v1")?;
        let conformer_etk_pipeline = pipeline(&device, library, "burette_conformer_etk_v1")?;
        let conformer_etk_optimize_pipeline =
            pipeline(&device, library, "burette_conformer_etk_optimize_v1")?;
        let mmff_energy_pipeline = pipeline(&device, library, "burette_mmff_energy_v1")?;
        let mmff_gradient_pipeline =
            pipeline(&device, library, "burette_mmff_analytic_gradient_v1")?;
        let mmff_optimize_pipeline = pipeline(&device, library, "burette_mmff_optimize_v1")?;
        let alignment_score_pipeline = pipeline(&device, library, "burette_alignment_score_v1")?;
        let rm1_fock_pipeline = pipeline(&device, library, "burette_rm1_pair_fock_v1")?;
        let pm6_pair_fock_pipeline = pipeline(&device, library, "burette_pm6_pair_fock_v1")?;
        let rm1_eigen_pipeline = pipeline(&device, library, "burette_rm1_symmetric_eigen_v1")?;
        let rm1_pair_rotate_pipeline = pipeline(&device, library, "burette_rm1_pair_rotate_v1")?;
        let pm6_h4_hh_pipeline = pipeline(&device, library, "burette_pm6_h4_hh_v1")?;
        let pm6_d3_pipeline = pipeline(&device, library, "burette_pm6_d3_v2")?;
        let pm6_one_center_fock_pipeline =
            pipeline(&device, library, "burette_pm6_one_center_fock_v1")?;
        Ok(Self {
        ...
        for row_start in (0..record_count).step_by(batch_capacity) {
            ...
            if let Some((offset, index)) = invalid {
                ...
                return Err(MetalRuntimeError::Dispatch(format!(
                    "Metal Tanimoto top-K returned invalid neighbors: row {row_index} rank {} \
                     selected index {index} (records {record_count}) with similarity {}",
                    offset % neighbors_per_vertex,
                    selected_similarities[offset],
                )));
            }
    ...
    pub(crate) fn contract_pm6_pair_fock_profiled(
        ...
        {
            return resource_limit(
                "PM6 Fock contraction requires a finite square density matrix through 256 orbitals",
            );
        }
        let mut descriptors = Vec::with_capacity(pairs.len().max(1));
        let mut repulsion = Vec::new();
        for (pair_index, pair) in pairs.iter().enumerate() {
            let expected = pair
                .left_orbital_count
                .checked_mul(pair.left_orbital_count)
                .and_then(|value| value.checked_mul(pair.right_orbital_count))
                .and_then(|value| value.checked_mul(pair.right_orbital_count));
            if expected != Some(pair.repulsion_ev.len())
                || !matches!(pair.left_orbital_count, 1 | 4 | 9)
                || !matches!(pair.right_orbital_count, 1 | 4 | 9)
                || pair.left_orbital_start + pair.left_orbital_count > orbital_count as usize
                || pair.right_orbital_start + pair.right_orbital_count > orbital_count as usize
                || pair.repulsion_ev.iter().any(|value| !value.is_finite())
            {
                return resource_limit(format!(
                    "PM6 Fock pair {pair_index} has an invalid orbital span or tensor"
                ));
            }
            descriptors.push(Pm6FockPairV1 {
                left_start: u32::try_from(pair.left_orbital_start)
                    .map_err(|_| memory_overflow())?,
                left_count: pair.left_orbital_count as u32,
                right_start: u32::try_from(pair.right_orbital_start)
                    .map_err(|_| memory_overflow())?,
                right_count: pair.right_orbital_count as u32,
                tensor_start: u32::try_from(repulsion.len()).map_err(|_| memory_overflow())?,
                reserved: 0,
            });
            repulsion.extend(pair.repulsion_ev.iter().map(|value| *value as f32));
        }
        ...
        if required_bytes > max_memory_bytes {
            return resource_limit(format!(
                "PM6 Fock contraction requires {required_bytes} accounted bytes; limit is {max_memory_bytes}"
            ));
        }
        let density_buffer = buffer_with_slice(&self.device, density);
        let pair_buffer = buffer_with_slice(&self.device, &descriptors);
        let repulsion_buffer = buffer_with_slice(&self.device, &repulsion);
        let output_buffer = buffer_with_slice(&self.device, &vec![0.0_f32; matrix_len]);
        let thread_width = self.pm6_pair_fock_pipeline.thread_execution_width().min(
            self.pm6_pair_fock_pipeline
                .max_total_threads_per_threadgroup(),
        );
        if thread_width == 0 {
            return Err(MetalRuntimeError::KernelUnavailable(
                "Metal PM6 Fock pipeline advertises zero thread width".into(),
            ));
        }
        let gpu_time = autoreleasepool(|| {
            let command = self.queue.new_command_buffer();
            let encoder = command.new_compute_command_encoder();
            encoder.set_compute_pipeline_state(&self.pm6_pair_fock_pipeline);
            encoder.set_buffer(0, Some(&density_buffer), 0);
            encoder.set_buffer(1, Some(&pair_buffer), 0);
            encoder.set_buffer(2, Some(&repulsion_buffer), 0);
            encoder.set_bytes(
        ...
        Ok(MetalPm6PairFockDispatch {
            contribution_ev: read_buffer(&output_buffer, matrix_len, "PM6 Fock contribution")?,
            gpu_time_seconds: gpu_time,
        })
    }
...
fn validate_alignment_atoms(
    label: &str,
    atoms: &[burette_compute_core::AlignmentAtom],
...
mod tests {
    ...
    use burette_compute_protocol::{ResourceLimits, MIN_COMPUTE_MEMORY_BYTES};
    ...
    fn dispatches_conformer_initialization_on_the_real_gpu() {
        ...
        let pipeline = pipeline(&device, &library, "burette_conformer_initialize_v1")

...
const server = createServer(async (request, response) => {
  try {
    ...
    if (request.url === '/') {
      ...
      response.setHeader('Content-Type', 'text/html');
      response.setHeader('Content-Security-Policy', `default-src 'none'; script-src 'unsafe-inline' 'wasm-unsafe-eval' blob:; style-src 'unsafe-inline' blob:; img-src data: blob:; font-src data: blob:; connect-src ${native ? 'blob:' : "'none'"}; worker-src blob:; frame-src blob:`);

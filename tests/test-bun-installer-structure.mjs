...
assert.match(source, /const LSREGISTER = '\/System\/Library\/Frameworks\/CoreServices\.framework\/Frameworks\/LaunchServices\.framework\/Support\/lsregister';/);
assert.match(source, /function registerInstalledApp\(targetApp\)/);
assert.match(source, /run\(LSREGISTER, \['-f', '-R', targetApp\]/);
assert.match(source, /run\('\/usr\/bin\/pluginkit', \['-a', quickLookExtension\]/);
assert.match(source, /run\('\/usr\/bin\/pluginkit', \['-e', 'use', '-i', QUICK_LOOK_EXTENSION_ID\]/);

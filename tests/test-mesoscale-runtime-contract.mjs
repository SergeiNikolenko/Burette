...
assert.match(runtime, /hostOwnedSelectors\.push\("\.burette-mesoscale-hosted \.msp-selection-viewport-controls"\)/);
assert.match(runtime, /window\.parent !== window/);
...
assert.match(contract, /MESOSCALE_SELECTION_BATCH_LIMIT = 4096/);
assert.match(contract, /MESOSCALE_SELECTION_REF_LIMIT = 256/);
...
assert.match(contract, /boundMesoscaleHierarchyPage/);
assert.match(contract, /MESOSCALE_HIERARCHY_DETAIL_DEPTH_LIMIT/);
assert.match(contract, /MESOSCALE_HIERARCHY_INSPECTION_LIMIT = 1024/);
assert.match(contract, /message\.source !== "burette-mesoscale-interaction"/);
for (const action of ["getSummary", "getHierarchyPage", "setSelection", "setSelectionBatch", "setSelectionStyle", "setSelectionVisibility", "isolateSelection", "setSelectionMode", "setIllumination", "setLayoutRegion", "setMotion", "setVisibility", "isolateObjects", "setStyle", "resetCamera", "orientAxes", "resetAxes", "exportState"]) {
...
assert.match(runtime, /selectEntities\(refs/);
assert.match(runtime, /Selection batch exceeds/);
assert.match(runtime, /const detailBudget = \{ remaining: MESOSCALE_HIERARCHY_DETAIL_LIMIT \}/);
assert.match(runtime, /refs\.length > MESOSCALE_SELECTION_BATCH_LIMIT/);
assert.match(runtime, /ref\.length > MESOSCALE_SELECTION_REF_LIMIT/);
assert.match(runtime, /childrenTruncated/);

...
function runCli(args) {
  return spawnSync(process.execPath, ['scripts/burette-agent.mjs', ...args], {
    encoding: 'utf8'
  });
}
...
try {
  const cliSource = await readFile(resolve('scripts/burette-agent.mjs'), 'utf8');
  ...
  assert.match(cliSource, /BURETTE_AGENT_SHELL_DIST_DIR/);
  ...
  assert.match(cliSource, /mkdtemp\(resolve\(tmpdir\(\), 'burette-agent-shell-'\)\)/);
  assert.match(cliSource, /BURETTE_AGENT_SHELL_SESSION_DIR: sessionDir/);
  assert.match(cliSource, /const logPath = resolve\(sessionDir, 'server\.log'\)/);
  assert.match(cliSource, /VITE_BURETTE_AGENT_SHELL: '1'/);
  ...
  assert.match(cliSource, /observe: `node scripts\/burette-agent\.mjs observe --session-dir/);
  assert.match(cliSource, /act: `node scripts\/burette-agent\.mjs act --session-dir/);
  ...
  try {
    const prebuiltDist = await mkdtemp(resolve(tmpdir(), 'burette-agent-shell-dist-'));
    await mkdir(resolve(prebuiltDist, 'assets'), { recursive: true });
    await writeFile(resolve(prebuiltDist, 'index.html'), '<!doctype html><title>Burette Agent Shell</title><main>ready</main>');
    try {
      for (const [mode, host] of [
        ...
        const invalidHost = runCliWithEnv(['open', '--mode', mode, '--host', host, 'samples/mini.pdb'], {
          BURETTE_AGENT_SHELL_DIST_DIR: prebuiltDist,
      ...
      const prebuiltShell = runCliWithEnv(['open', '--mode', 'browser-agent-shell', 'samples/mini.pdb', 'samples/mini.cif'], {
        BURETTE_AGENT_SHELL_DIST_DIR: prebuiltDist,
        PATH: `${fakeBin}:${process.env.PATH}`,
      });
      assert.equal(prebuiltShell.status, 0, prebuiltShell.stderr);
      const prebuiltPayload = JSON.parse(prebuiltShell.stdout);
      assert.deepEqual(prebuiltPayload.result.initialPaths, [resolve('samples/mini.pdb'), resolve('samples/mini.cif')]);
      assert.equal(new URL(prebuiltPayload.result.url).searchParams.get('devFiles'), prebuiltPayload.result.initialPaths.join('\n'));
      ...
      for (const invalidUrl of [
        ...
        const invalidSessionDir = await mkdtemp(resolve(tmpdir(), 'burette-agent-invalid-session-'));
        try {
      ...
      const staleAssetDist = await mkdtemp(resolve(tmpdir(), 'burette-agent-stale-shell-dist-'));
      try {
        ...
        const staleAssetShell = runCliWithEnv(['open', '--mode', 'browser-agent-shell', 'samples/mini.pdb'], {
          BURETTE_AGENT_SHELL_DIST_DIR: staleAssetDist,
          PATH: `${fakeBin}:${process.env.PATH}`,
        });
    ...
    const failedShell = runCliWithEnv(['open', '--mode', 'browser-agent-shell', 'samples/mini.pdb'], {
      BURETTE_AGENT_SHELL_FORCE_VP: '1',
      PATH: `${fakeBin}:${process.env.PATH}`,
    });

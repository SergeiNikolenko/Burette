...
assert.match(textViewer, /onStructureSelectionRef\.current\?\.\(documentRef\.current, selection\)/);
...
assert.match(textFilesCommand, /pub\(crate\) async fn open_text_files/);
assert.match(textFilesCommand, /pub\(crate\) async fn read_text_file\(\s*path: String,\s*max_bytes: Option<usize>,\s*\)/);
assert.match(textFilesCommand, /spawn_blocking/);
assert.match(textFilesCommand, /const TEXT_BATCH_READ_LIMIT: usize = 64 \* 1024 \* 1024;/);
...
assert.match(fileRouting, /preferredTextBasenames = new Set\(\[[\s\S]*"log\.lammps"[\s\S]*\]\)/);
assert.match(fileRouting, /export function isPreferredTextPath\(path: string, extension = pathExtension\(path\)\)/);
...
assert.match(appFileOpen, /const result = await openDocuments\(structurePaths, undefined, \{ rendererMode: "auto" \}\);/);
assert.match(appFileOpen, /const result = await openDocuments\(structureAndTextPaths, undefined, \{ rendererMode: "auto" \}, \{ deferErrorStatus: true \}\);/);
...
assert.match(appDockPayloadOpen, /const rightDockContentSpectrumPaths = await detectContentSpectrumPaths\(unopenedPaths\);/);
assert.match(appDockPayloadOpen, /const rightDockTextPaths = unopenedPaths\.filter\(\(path\) => \{/);

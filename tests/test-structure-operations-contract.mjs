...
// A new combined scene shows all structures, while an explicit saved choice wins.
const readPoseMode = new Function('dockingSceneMode', 'poseModeStorageKey', 'window',
  `return (${fn('readSdfPoseMode')});`);
for (const stored of [null, 'single', 'all']) {
  const mode = readPoseMode(config => config.sceneMode, () => 'fixture', { localStorage: { getItem: () => stored } });
  assert.equal(mode({ sceneMode: 'structureAll' }), stored || 'all');
  assert.equal(mode({ sceneMode: 'structurePoses' }), stored || 'single');
}

// The shared viewer keeps desktop compute, but the agent plugin is visualization-only.
const canCompute = new Function("normalizeFormat", `return (${fn("canGenerate3DConformerFromConfig")});`)(value => value);
assert.equal(canCompute({ format: "sdf" }, "molstar"), true);
assert.equal(canCompute({ format: "sdf", visualizationOnly: true }, "molstar"), false);
assert.equal(canCompute({ format: "mol", visualizationOnly: true }, "molstar"), false);

...
assert.match(shellCommand, /pub\(crate\) struct ProjectRootWatcher/);
assert.match(shellCommand, /RecommendedWatcher/);
assert.match(shellCommand, /RecursiveMode::Recursive/);
assert.match(shellCommand, /pub\(crate\) fn watch_project_roots/);
assert.match(lib, /manage\(commands::shell::ProjectRootWatcher::default\(\)\)/);
assert.match(lib, /commands::shell::watch_project_roots/);
assert.match(tauriPermissionSource, /"watch_project_roots"/);
...
assert.match(updaterCommand, /"\$LSREGISTER" -u "\$NEW_APP"/);
...
assert.match(releaseScript, /BURETTE_RELEASE_ALLOW_ADHOC:-auto/);
assert.match(releaseScript, /developer_id_credentials_available/);
assert.match(releaseScript, /building an ad-hoc signed release without notarization/);

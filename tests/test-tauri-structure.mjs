...
assert.match(releaseWorkflow, /Require Developer ID release secrets/);
assert.match(releaseWorkflow, /BURRETE_BUILD_MODE: release/);
assert.doesNotMatch(releaseWorkflow, /BURRETE_RELEASE_ALLOW_ADHOC/);
...
assert.equal(packageConfig.scripts['test:update'], 'bun tests/test-update-versioning.mjs && bun tests/test-bun-installer-structure.mjs && bun tests/test-bun-installer-behavior.mjs && bun tests/test-runner-contract.mjs && bun tests/test-dev-namespace.mjs');

...
async function testMockedWorkspaceToolScenarios(tempRoot) {
  ...
  for (const name of ['burette.open_workspace', 'open_burette_workspace']) {
    for (const scene of [undefined, 'structureAll']) {
      const result = await server.tools.get(name).handler({
        file: sampleMini, additionalFiles: [sample1htb], mode: 'browser-agent-shell', scene, noLaunch: true,
      });
      assert.deepEqual(result.structuredContent.result.initialPaths, [sampleMini, sample1htb]);
      assert.deepEqual(result.structuredContent.result.argv, [
        '--mode', 'browser-agent-shell', '--no-launch', sampleMini, sample1htb,
        ...(scene ? ['--scene', scene] : []),
      ]);
    }
  }

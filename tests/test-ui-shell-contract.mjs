...
assert.doesNotMatch(commandPalette, /DialogTitle/);
assert.doesNotMatch(commandPalette, /DialogDescription/);
...
assert.match(appUpdatesHook, /const installUpdate = useCallback\(async \(releaseOverride\?: UpdateRelease \| null\) => \{\s*if \(buildInfo\.isBrowserDev\) \{\s*pushStatus\("Updates are disabled in browser sessions\."\);\s*return;\s*\}/s);
assert.match(appUpdatesHook, /const promptForUpdate = useCallback\(async \(release: UpdateRelease, automatic: boolean\) => \{\s*if \(buildInfo\.isBrowserDev\) return;\s*if \(!shouldPromptForUpdate\(release, automatic\)\) return;/s);
...
assert.match(appUpdatesHook, /const openUpdateRelease = useCallback\(async \(\) => \{\s*if \(buildInfo\.isBrowserDev\) \{\s*pushStatus\("Updates are disabled in browser sessions\."\);\s*return;\s*\}/s);
assert.match(appBootstrapHook, /statusText: info\.isBrowserDev \? "Updates are disabled in browser sessions\." : "Updates are disabled for dev builds\."/);

...
assert.match(structureInfoPanel, /const STRUCTURE_SCENE_STYLE_OPTIONS = \[/);
assert.match(structureInfoPanel, /\{ value: "illustrative", label: "Colorful" \}/);
...
assert.match(structureInfoPanel, /const STRUCTURE_SCENE_OPACITY_MAX = 1/);
assert.match(structureInfoPanel, /const isMolstarDocument = document\.renderer === "molstar"/);
assert.match(structureInfoPanel, /const hasStructureScene = isMolstarDocument && Boolean\(document\.dockingRequest\?\.sceneMode\)/);
assert.match(structureInfoPanel, /const hasStructureSceneAllMode = isMolstarDocument && document\.dockingRequest\?\.poseMode === "all"/);
assert.match(structureInfoPanel, /const hasStructureFrameAllMode = isMolstarDocument && shouldShowStructureFrameStyleCard\(document, compositionSummary\)/);
assert.match(structureInfoPanel, /const allStructureStyleCard = hasStructureSceneAllMode/);
assert.match(structureInfoPanel, /title: "All frames"/);
assert.match(structureInfoPanel, /ariaLabel: "All frames style"/);
assert.match(structureInfoPanel, /opacityLabel: "All frames opacity"/);
assert.match(structureInfoPanel, /const sceneControls = document\.renderer === "molstar" && Boolean\(document\.dockingRequest\?\.sceneMode \|\| isStructureFrameStyleDocument\(document\)\)/);
assert.match(structureInfoPanel, /readSdfContextStylePreference\(document, sceneControls \? STRUCTURE_SCENE_STYLE_OPTIONS : SDF_CONTEXT_STYLE_OPTIONS\)/);
assert.match(structureInfoPanel, /function SdfContextStyleCard/);
assert.match(structureInfoPanel, /title = "All background"/);
assert.match(structureInfoPanel, /detail = "Context molecules"/);
assert.match(structureInfoPanel, /title=\{allStructureStyleCard\.title\}/);
assert.match(structureInfoPanel, /detail="All-together view"/);
assert.match(structureInfoPanel, /ariaLabel=\{allStructureStyleCard\.ariaLabel\}/);
assert.match(structureInfoPanel, /opacityLabel=\{allStructureStyleCard\.opacityLabel\}/);
assert.match(structureInfoPanel, /styleOptions=\{STRUCTURE_SCENE_STYLE_OPTIONS\}/);
assert.match(structureInfoPanel, /opacityDisabled=\{sdfContextStyle === "illustrative"\}/);
assert.match(structureInfoPanel, /value=\{normalizeStructureSceneStyle\(sdfContextStyle\)\}/);
assert.match(structureInfoPanel, /opacityMax=\{STRUCTURE_SCENE_OPACITY_MAX\}/);
assert.match(structureInfoPanel, /function isStructureFrameStyleDocument\(document: ViewerDocument\)/);
assert.match(structureInfoPanel, /document\.renderer === "molstar" && \(extension === "xyz" \|\| extension === "extxyz"\)/);
assert.match(structureInfoPanel, /function shouldShowStructureFrameStyleCard\(document: ViewerDocument, summary: StructureCompositionSummary \| null\)/);
assert.match(structureInfoPanel, /summaryInteger\(valueForLabel\(summary\?\.rows \?\? \[\], "Frames"\)\) > 1/);
assert.match(structureInfoPanel, /function summaryInteger\(value: string \| null\)/);
assert.match(structureInfoPanel, /function normalizeStructureSceneStyle\(value: SdfContextStyle\)/);
assert.match(structureInfoPanel, /function normalizeContextStyleForOptions\(/);
assert.match(structureInfoPanel, /return styleOptions\.some\(\(option\) => option\.value === value\) \? value as SdfContextStyle : "line"/);
assert.match(structureInfoPanel, /function StructureSceneControlsCard/);
assert.match(structureInfoPanel, /StructureSectionHeader title="Scene structures" detail=\{`\$\{paths\.length\} structures`\}/);
...
assert.match(structureInfoPanel, /opacityLabel = "All background opacity"/);
assert.match(structureInfoPanel, /aria-label=\{opacityLabel\}/);
assert.match(structureInfoPanel, /\{opacityDisabled \? null : \(/);
...
assert.match(previewViewer, /await plugin\.builders\.structure\.hierarchy\.applyPreset\(trajectory, 'default', \{ representationPreset: 'empty' \}\)/);

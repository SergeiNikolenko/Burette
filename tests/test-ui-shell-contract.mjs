...
assert.match(appShellNavigationActionsHook, /const focusSidebarSearch = useCallback\(\(\) => \{\s*openCommandPalette\("search"\);/);
...
assert.doesNotMatch(bootOverlayScript, /function hasMountedApp\(\)/);
...
assert.match(agentPreviewViewer, /String\(body\.documentId\) !== documentId && !hasXyzrenderArtifact/);
assert.match(agentPreviewViewer, /renderer !== 'xyzrender-external' && !hasXyzrenderArtifact/);
assert.match(agentPreviewViewer, /function xyzrenderOrientationPayload\(\)/);
assert.match(agentPreviewViewer, /postHostMessage\(\{ type: 'setXyzrenderControls', documentId, controls, preset, \.\.\.xyzrenderOrientationPayload\(\) \}\)/);
...
const initStaticRendererToolbarBody = viewer.match(/function initStaticRendererToolbar\(\) \{([\s\S]*?)\n  \}/)?.[1] || "";
...
assert.match(dockPanel, /const descriptorDockAvailable = !descriptorDockBlocked\(area, activeStructureDocument\)/);
assert.match(dockPanel, /if \(tab\.kind === "descriptors"\) return descriptorDockAvailable && !\(area === "right" && spectrumDocumentActive\)/);
assert.match(dockPanel, /if \(kind === "descriptors"\) return descriptorDockAvailable && !\(area === "right" && spectrumDocumentActive\)/);
assert.match(dockPanel, /function descriptorDockBlocked\(area: DockArea, document: ViewerDocument \| null \| undefined\)/);
assert.match(dockPanel, /Boolean\(document\.dockingRequest\) \|\| \(document\.virtual && document\.renderer === "molstar"\)/);
...
assert.match(dockPanel, /const xyzrenderDockDocument = area === "right" \? \(state\.activeDocument \?\? dockDocument\) : null/);
assert.match(dockPanel, /const xyzrenderDocument = area === "right" \? activeDocument \?\? dockDocument : dockStructureDocument/);
assert.doesNotMatch(dockPanel, /dockStructureDocument\?\.renderer === "xyzrender-external"/);
...
assert.match(dockPanel, /const XYZRENDER_README_HULL_OPTIONS = \[[\s\S]*?Off[\s\S]*?caffeine_default\.svg[\s\S]*?Rings[\s\S]*?anthracene_hull\.svg[\s\S]*?Faces[\s\S]*?buckyball_faces\.svg/);
assert.match(dockPanel, /const XYZRENDER_README_PORE_OPTIONS = \[[\s\S]*?Pore[\s\S]*?buckyball_pore\.svg[\s\S]*?Faces \+ pore[\s\S]*?mof5_faces_pore\.svg/);
assert.doesNotMatch(dockPanel, /Benzene ring/);
assert.doesNotMatch(dockPanel, /Anthracene rings/);
assert.doesNotMatch(dockPanel, /Buckyball faces/);
assert.doesNotMatch(dockPanel, /MOF-5 faces/);
...
assert.match(dockPanel, /<XyzrenderPresetGallery[\s\S]*?preset=\{preset\}[\s\S]*?onSelect=\{\(value\) => \{[\s\S]*?setPresetState\(value\);[\s\S]*?apply\(controlsRef\.current, value\);[\s\S]*?\}\}/);
assert.match(dockPanel, /<XyzrenderDisplayOptionsGallery[\s\S]*?controls=\{controls\}[\s\S]*?onSelect=\{\(nextControls\) => \{[\s\S]*?setControlsState\(nextControls\);[\s\S]*?apply\(nextControls, presetRef\.current\);[\s\S]*?\}\}/);
assert.match(dockPanel, /<XyzrenderVdwGallery[\s\S]*?controls=\{controls\}[\s\S]*?onSelect=\{\(mode\) => \{[\s\S]*?const currentControls = controlsRef\.current[\s\S]*?const selectedMode = currentControls\.showVdw === true[\s\S]*?currentControls\.vdwAtoms \? "partial" : "all"[\s\S]*?if \(mode === "off" \|\| mode === selectedMode\) \{[\s\S]*?showVdw: false[\s\S]*?vdwAtoms: null[\s\S]*?apply\(nextControls, presetRef\.current\)[\s\S]*?showVdw: true[\s\S]*?apply\(nextControls, presetRef\.current, mode === "partial" \? \{ xyzrenderSelectionAction: "vdw" \} : \{\}\)/);
...
assert.match(agentIntegrationPanel, /Open Install/);
assert.match(agentIntegrationPanel, /Copy Path/);
...
assert.match(styles, /\.project-folder-row \{[^}]*position: relative;[^}]*padding: 5px 34px 5px calc\(6px \+ \(var\(--project-depth, 0\) \* 6px\)\);/s);
...
assert.match(appShellActionsHook, /\.\.\.createProjectShellActions\(\{ pushStatus, removeProjectRoot, renameProjectRoot, togglePinnedProjectRoot \}\)/);
...
assert.match(tauriSource, /void result\.catch/);
...
assert.match(app, /useAppSpectrumDockLifecycle\(\{[\s\S]*activeDocument,[\s\S]*bottomDockActiveTab,[\s\S]*bottomDockDocumentId,[\s\S]*bottomDockTabs,[\s\S]*closeDockTab,[\s\S]*documents,[\s\S]*rightDockActiveTab,[\s\S]*rightDockDocumentId,[\s\S]*setDockActiveTab,[\s\S]*setDockDocument,[\s\S]*setDockOpen,[\s\S]*\}\)/);
...
assert.match(browserDevDocuments, /extension === "lammpstrj" \|\| extension === "dump"[\s\S]*return bytes \? \{ bytes, molstarFormat: "xyz" \} : null/);
...
assert.match(previewViewer, /nativeTrajectoryControls: false,\s*poseCount: frames\.length,\s*controlLabel: 'Frame',\s*xyzFrameMode: 'single'/);
for (const runtimeSource of [previewViewer, agentPreviewViewer]) {
  ...
  assert.match(runtimeSource, /const stateKey = xyzFrameOverlayStateKey\(rawSignature, frames, prepared, style, resolvedContextStyle, contextOpacity, contextColor, backgroundIndexes\)/);
...
assert.match(previewViewer, /if \(prepared\.xyzFrameOverlayAvailable === true\) \{\s*await applyXyzFrameOverlayVisibility\(viewer, prepared, readTrajectoryControlIndex\(activeConfig, prepared, prepared\.poseCount \|\| prepared\.xyzFrameCount\)\);\s*return;\s*\}/);
...
assert.match(previewViewer, /if \(prepared\.xyzFrameOverlayAvailable === true\) \{\s*await applyXyzFrameOverlayVisibility\(activeViewer, prepared, index\);\s*\} else if \(prepared\.nativeTrajectoryControls\) \{/);
...
assert.match(previewViewer, /const scheduleLoopStep = \(delayMs = prepared\.nativeTrajectoryControls \? loopDelayMs\(\) : loopNextDelay\(\)\) => \{/);
assert.match(previewViewer, /loopTimer = window\.setTimeout\(\(\) => \{/);
assert.match(previewViewer, /const nextIndex = prepared\.nativeTrajectoryControls\s*\?\s*\(activePose \+ 1\) % prepared\.poseCount\s*:\s*loopTargetIndex\(\)/);
...
assert.match(previewViewer, /if \(prepared\.nativeTrajectoryControls \|\| prepared\.kind === 'sdf-collection' \|\| prepared\.kind === 'xyz-frame-overlay'\) scheduleSliderInputPose\(previewIndex\)/);
...
assert.match(sidebarFileTreeNode, /className="project-group-row"[\s\S]*draggable=\{project\.items\.length > 0\}/);
...
assert.match(appDockingWorkflowsHook, /if \(rightDockOpen && rightDockActiveTab === "descriptors"\) \{\s*setDockOpen\("right", false\);\s*\}/);
assert.doesNotMatch(appDockingWorkflowsHook, /request\.sceneMode && rightDockOpen && rightDockActiveTab === "descriptors"/);

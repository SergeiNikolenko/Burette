...
assert.match(viewer, /function installExternalArtifactCardInteractions\(root\)/);
...
assert.match(viewer, /event\.target\?\.\s*closest\?\.\('\.buret-xyzrender-sheet-rotate-handle'\)/);
...
assert.match(browserDevDocuments, /const GRID_ASSET_VERSION = "grid-ui-v64"/);
assert.match(browserDevDocuments, /const VIEWER_ASSET_VERSION = "viewer-ui-v12"/);
...
assert.match(previewViewer, /\.buret-external-artifact-card \{[^}]*transform: translate\(var\(--buret-card-x\), var\(--buret-card-y\)\) rotate\(var\(--buret-sheet-rotation\)\);/s);

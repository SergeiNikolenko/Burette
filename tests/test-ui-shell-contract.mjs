...
assert.match(viewer, /const XYZRENDER_GALLERY_PRESETS = \[[\s\S]*?'default'[\s\S]*?'flat'[\s\S]*?'paton'[\s\S]*?'pmol'[\s\S]*?'skeletal'[\s\S]*?'bubble'[\s\S]*?'tube'[\s\S]*?'btube'[\s\S]*?'wire'[\s\S]*?'graph'[\s\S]*?'mtube'[\s\S]*?'vdw'/);
assert.match(viewer, /function appendXyzrenderPresetGallery\(menu, currentPreset\)/);
assert.match(viewer, /className = 'buret-xyzrender-preset-grid'/);
assert.match(viewer, /button\.dataset\.buretXyzrenderPreset = value/);
assert.match(viewer, /requestXyzrenderPreset\(value\)/);
assert.match(viewer, /appendXyzrenderPresetGallery\(menu, xyzrenderSheetItemPreset\(item\)\)/);
...
assert.match(previewViewController, /if preset != "vdw", \(controls\["showVdw"\] as\? Bool\) == true \{\s*arguments\.append\("--vdw"\)/);
...
assert.match(viteConfig, /if \(preset !== "vdw" && controls\.showVdw === true\) args\.push\("--vdw"\)/);

...
assert.match(styles, /\.workspace \{[^}]*display: flex;[^}]*overflow: hidden;/s);
assert.match(styles, /\.sidebar-shell \{[^}]*position: absolute;[^}]*width: var\(--sidebar-layout-width, 0px\);[^}]*pointer-events: none;[^}]*z-index: 2;/s);
assert.doesNotMatch(styles, /\.sidebar-shell \{[^}]*transition: width 140ms ease-out/s);
assert.match(styles, /\.sidebar-shell-inner \{[^}]*height: 100%;[^}]*transform: translateX\(-16px\);[^}]*transition: transform 140ms ease-out, opacity 100ms ease-out;/s);
assert.match(styles, /\.app-shell\[data-sidebar-open="true"\] \.sidebar-shell-inner \{[^}]*opacity: 1;[^}]*transform: translateX\(0\);/s);
assert.match(styles, /\.app-shell\[data-sidebar-open="true"\] \.sidebar-shell \{[^}]*pointer-events: auto;/s);
assert.match(styles, /\.splitter \{[^}]*position: absolute;[^}]*left: calc\(var\(--sidebar-layout-width, 0px\) - 4px\);[^}]*flex: none;[^}]*margin: 0;/s);

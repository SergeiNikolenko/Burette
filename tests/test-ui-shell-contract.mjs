...
assert.match(structureInfoPanel, /const textPromise = virtualText === null \? readStructureText\(document\.path\) : Promise\.resolve\(virtualText\);/);
...
assert.match(dockPanel, /const textPromise = virtualText === null \? readStructureText\(activeDocument\.path\) : Promise\.resolve\(virtualText\);/);
...
assert.match(dockPanel, /const KetcherPage = lazy\(\(\) => import\("\.\/ketcher-page"\)/);
assert.match(dockPanel, /<Suspense fallback=\{<div className="ketcher-loading">Loading editor<\/div>\}>/);
...
assert.match(ketcherPage, /await navigator\.clipboard\.writeText\(text \|\| ""\)/);

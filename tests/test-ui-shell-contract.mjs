...
assert.match(gridViewer, /target\.closest\('path, line, circle, ellipse, polygon, polyline, text, image'\)/);
...
assert.match(gridViewer, /try \{\s*await initRDKit\(\);\s*state\.rdkitError = '';\s*\} catch \(rdkitError\) \{/s);

...
for (const runtimeSource of [viewer]) {
  ...
  assert.match(runtimeSource, /async function applyMolstarLassoPicks\(picks, additive\)/);
  assert.match(runtimeSource, /const MOLSTAR_LASSO_SAMPLE_LIMIT = 1400;/);
  assert.match(runtimeSource, /const adaptiveStep = Math\.ceil\(Math\.sqrt\(\(bounds\.width \* bounds\.height\) \/ gridBudget\)\)/);
  assert.match(runtimeSource, /if \(sampled >= MOLSTAR_LASSO_SAMPLE_LIMIT\) break sampleGrid;/);
  assert.match(runtimeSource, /await molstarLassoYield\(\)/);
  assert.match(runtimeSource, /const lassoApplyGranularity = false;/);
  assert.match(runtimeSource, /StructureElement\.Loci\.union\(previous, loci\)/);
  assert.match(runtimeSource, /for \(const loci of mergedByStructure\.values\(\)\)/);
  ...
  assert.match(runtimeSource, /function notifyMolstarLassoSelection\(picks, batchAtoms, totalAtoms\)/);

...
assert.match(editorTabs, /onMouseDown=\{\(event\) => \{\s*if \(event\.button === 2\) \{\s*showTabMenu\(event\);\s*return;\s*\}\s*startMouseTabReorder\(tab\.id, event\);/s);
...
assert.match(editorTabs, /startMouseTabReorder\(tab\.id, event\)/);

...
assert.match(appViewerReloadActionsHook, /activeViewerIframeForDocument\(document\.id, document\.renderer\)/);
...
assert.match(viewer, /appendXyzrenderMenuButton\(actions, 'Undo'/);
assert.match(viewer, /appendXyzrenderMenuButton\(actions, 'Redo'/);
...
assert.match(viewer, /function undoXyzrenderLastAction\(\)/);
assert.match(viewer, /function redoXyzrenderLastAction\(\)/);
...
assert.match(previewViewer, /document\.addEventListener\('keydown'[\s\S]*key !== 'z'[\s\S]*!\(event\.metaKey \|\| event\.ctrlKey\)[\s\S]*undoMolstarLastEdit\(\)/);

...
assert.doesNotMatch(app, /refreshedPersistedSessionRef/);
...
assert.doesNotMatch(app, /openedPersistedTabsRef/);
assert.match(app, /useOpenEvents\(\s*\(paths, options\) => openDocuments\(paths, undefined, undefined, options\),\s*pushErrorStatus,\s*\)/);
...
assert.doesNotMatch(app, /typeof path === "string" && !isTemporaryDocumentPath\(path\)/);
assert.match(app, /pinnedStructurePaths,/);
assert.match(app, /sidebarProjects/);
assert.doesNotMatch(app, /isTauriRuntime\(\) \|\| documents\.length === 0/);
assert.doesNotMatch(app, /void openDocuments\(paths\)/);

...
assert.match(structureInfoPanel, /StructureSectionHeader title="All background" detail="Context molecules"/);
...
assert.match(structureInfoPanel, /aria-label="All background opacity"/);
...
assert.match(browserDevDocuments, /const GRID_ASSET_VERSION = "grid-ui-v101"/);
...
assert.match(previewViewer, /if \(activeSdfPoseMode === 'all'\) \{/);
...
assert.match(gridViewer, /\['molstar', 'Open in Mol\*'\]/);
assert.match(gridViewer, /\['ketcher', 'Edit in Ketcher'\]/);
assert.match(gridViewer, /\['generate3d', 'Generate 3D'\]/);

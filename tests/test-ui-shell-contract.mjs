...
assert.match(viewer, /document\.addEventListener\('click', onXyzrenderLassoClearClick, true\)/);
assert.match(viewer, /function onXyzrenderLassoClearClick\(event\)/);
assert.match(viewer, /if \(!stroke\.dragging\) \{\n      xyzrenderLassoStroke = null;\n      clearXyzrenderSelection\(\);/);
assert.match(viewer, /xyzrenderLassoSuppressClickUntil = Date\.now\(\) \+ 250/);
assert.match(viewer, /Date\.now\(\) < xyzrenderLassoSuppressClickUntil/);

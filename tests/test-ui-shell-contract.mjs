...
assert.match(gridViewer, /Math\.max\(12, Math\.min\(120, Math\.floor\(value\)\)\)/);
...
assert.match(gridViewer, /const BACKGROUND_CARD_RENDER_BATCH = 16;/);
assert.match(gridViewer, /const BACKGROUND_GRID_WORK_DELAY_MS = 120;/);
assert.match(gridViewer, /const RDKIT_CARD_FRAME_BATCH = 6;/);
assert.match(gridViewer, /const RDKIT_CARD_FRAME_BUDGET_MS = 8;/);
assert.match(gridViewer, /const RDKIT_SVG_CACHE_LIMIT = 600;/);
...
assert.match(gridViewer, /backgroundGridWorkTimer: 0/);
assert.match(gridViewer, /function compareCardRenderJobs\(a, b\)/);
assert.match(gridViewer, /function cardRenderPriority\(target\)/);
assert.match(gridViewer, /function scheduleBackgroundGridWork\(cfg, token = state\.token\)/);
assert.match(gridViewer, /function runBackgroundGridWork\(cfg, token\)/);
...
assert.match(gridViewer, /function startBackgroundCardRenderJobs\(limit\)/);
assert.match(gridViewer, /function startBackgroundRdkitCards\(limit\)/);
assert.match(gridViewer, /function startBackgroundXyzrenderCards\(limit\)/);
assert.match(gridViewer, /function hasBackgroundGridWork\(\)/);
...
assert.doesNotMatch(gridCss, /buret-grid-rail-popover/);
assert.doesNotMatch(gridCss, /buret-grid-rail-hover-target/);
assert.doesNotMatch(gridCss, /buret-card-rail-hover/);
assert.doesNotMatch(gridViewer, /buret-grid-rail-popover/);
...
assert.match(gridViewer, /if \(Number\.isFinite\(position\)\) void scrollToGridPosition\(position, cfg\);\s*else scrollToGridRow\(index, cfg\);/);
...
assert.doesNotMatch(welcome, /Burrete Desktop/);
assert.doesNotMatch(welcome, /Open a molecular structure/);

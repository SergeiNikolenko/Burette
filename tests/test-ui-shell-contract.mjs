...
assert.match(openDropHook, /addXyzrenderSheetItems\?\.\(event\.paths\)/);
assert.match(openDropHook, /if \(addXyzrenderSheetItems\?\.\(paths\)\) return;/);
...
assert.match(browserDevDocuments, /const VIEWER_ASSET_VERSION = "viewer-ui-v14"/);
...
assert.match(previewViewer, /new ResizeObserver\(\(\) => updateExternalArtifactCardFit\(card\)\)/);
...
assert.match(fileKind, /const droppedPaths = readStructureDrag\(event\.dataTransfer\)/);
assert.match(fileKind, /if \(postXyzrenderSheetItems\(droppedPaths\)\) return/);

...
assert.match(dockPanel, /const XYZRENDER_README_VDW_OPTIONS = \[[\s\S]*?asparagine_vdw\.svg[\s\S]*?asparagine_vdw_partial\.svg[\s\S]*?asparagine_vdw_paton\.svg/);
...
assert.match(dockPanel, /<XyzrenderVdwGallery[\s\S]*?controls=\{controls\}[\s\S]*?preset=\{preset\}[\s\S]*?onSelect=\{\(mode\) => \{[\s\S]*?xyzrenderSelectionAction: "vdw"/);

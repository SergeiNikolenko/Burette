...
assert.match(dockPanel, /hullOpacity: nextMode === "off" \? null : \(controls\.hullOpacity \?\? 0\.45\)/);
assert.match(dockPanel, /poreOpacity: nextMode === "off" \? null : \(controls\.poreOpacity \?\? 0\.6\)/);
...
assert.match(previewViewer, /if \(prepared\?\.xyzFrameOverlayAvailable === true\) \{[\s\S]*?kind: 'xyz-frame-overlay'/);
assert.doesNotMatch(previewViewer, /prepared\?\.xyzFrameOverlayAvailable === true && activeSdfPoseMode === 'all'/);

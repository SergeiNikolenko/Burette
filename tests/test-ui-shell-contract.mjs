...
assert.match(moleculeStore, /documents: \[\],\s*tabs: persistedTabs\(state\.tabs, state\.documents\)/);
...
assert.match(commandDocuments, /pub\(crate\) fn render_xyzrender_sheet_item/);
...
assert.match(browserDevDocuments, /const GRID_ASSET_VERSION = "grid-ui-v84"/);
...
assert.match(gridViewer, /function xyzrenderCardImageHTML\(svg, row\)/);
assert.match(gridViewer, /class="buret-xyzrender-card-image"/);

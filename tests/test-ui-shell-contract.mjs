...
assert.match(browserDevDocuments, /const shouldOpenXyzTrajectoryInMolstar = xyzFrameCount > 1 && requestedMode === "auto";/);
...
assert.match(gridViewer, /Math\.max\(12, Math\.min\(48, Math\.floor\(value\)\)\)/);
assert.match(browserDevDocuments, /pageSize: 48/);
...
assert.match(gridViewer, /const XYZRENDER_CARD_ROOT_MARGIN = '720px 0px';/);
...
assert.match(gridViewer, /class="buret-xyzrender-preview" data-buret-xyzrender-card-key/);
assert.match(gridViewer, /target\.classList\.remove\('buret-xyzrender-preview'\)/);
...
assert.match(gridCss, /\.buret-grid-toolbar \{[^}]*backdrop-filter: blur\(14px\) saturate\(135%\);/s);
...
assert.match(gridCss, /\.buret-grid \{[^}]*padding-inline: 24px;/s);
assert.doesNotMatch(gridCss, /\.buret-grid \{[^}]*justify-content: start;/s);
assert.doesNotMatch(gridCss, /\.buret-grid \{[^}]*padding-left: 24px;/s);
assert.match(gridViewer, /data-buret-grid-rail-popover data-state="closed" role="dialog" aria-label="Molecule navigation" aria-hidden="true" hidden inert/);
assert.match(gridViewer, /applyGridPreferences\(\);\s*initGridRail\(cfg\);\s*initInfiniteLoading\(cfg\);/);
assert.match(gridViewer, /setGridRailOpen\(false\);\s*if \(Number\.isFinite\(position\)\) scrollToGridPosition\(position, cfg\);/);
assert.match(gridViewer, /popover\.setAttribute\('aria-hidden', open \? 'false' : 'true'\);/);
assert.match(gridViewer, /popover\.hidden = !open;/);
...
assert.match(styles, /\.ketcher-editor-root,\s*\.ketcher-editor-root > \[class\*="Editor-module_editor"\],\s*\.ketcher-editor-shell \[class\*="Editor-module_editorsWrapper"\],\s*\.ketcher-editor-shell \[class\*="App-module_app"\],\s*\.ketcher-editor-shell \[class\*="App-module_top"\],\s*\.ketcher-editor-shell \[class\*="App-module_canvas"\]\s*\{[^}]*min-width: 0;[^}]*max-width: 100%;/s);
assert.match(styles, /\.ketcher-editor-root\s*\{[^}]*width: calc\(100% \* var\(--ketcher-ui-scale-inverse, 1\)\);[^}]*height: calc\(100% \* var\(--ketcher-ui-scale-inverse, 1\)\);[^}]*transform: scale\(var\(--ketcher-ui-scale, 1\)\);/s);
...
assert.match(ketcherPage, /const KETCHER_UI_SCALES = \[0\.76, 0\.82, 0\.88, 0\.94, 1\] as const;/);
...
assert.match(ketcherPage, /"--ketcher-ui-scale-inverse": String\(1 \/ ketcherUIScale\)/);
...
assert.match(ketcherPage, /disabled=\{!ketcher \|\| exportingSketch\} onClick=\{\(\) => void openSketch\("molstar"\)\}/);
...
assert.match(ketcherEditor, /getMolfile: async \(\) => serializeCurrentMolfile\(instance, MolSerializer\)/);
...
assert.match(app, /pushStatus\("Opening separate Mol\* docking view\.\.\."\)/);
...
assert.match(browserDevDocuments, /const GRID_ASSET_VERSION = "grid-ui-v89"/);
assert.match(browserDevDocuments, /const VIEWER_ASSET_VERSION = "viewer-ui-v17"/);
...
assert.match(previewShell, /data-buret-renderer="xyz-fast"/);
...
assert.match(previewViewer, /if \(value === 'xyz-fast'\) \{\s*void switchBrowserDevXyzFast\(\);\s*return true;\s*\}/s);
assert.match(previewViewer, /async function switchBrowserDevXyzFast\(\)/);
...
assert.match(previewViewer, /await ensureBrowserDevStructureData\(nextConfig, cb\);\s*await startXYZFast\(nextConfig, cb\);/s);
...
assert.match(browserDevDocuments, /if \(normalized === "xyz-fast"\) return isXyz \? "xyz-fast" : "molstar";/);
...
assert.match(previewViewer, /const DOCKING_POSE_POSITION_VERSION = '2'/);
...
assert.match(gridViewer, /state\.cardRenderer !== 'rdkit' \|\| !hasInputCoordinateRows\(\)/);
assert.match(gridViewer, /function hasMolblockInputCoordinates\(value\)/);
assert.match(gridViewer, /Math\.abs\(z\) > 1e-6/);
...
assert.match(gridCss, /\.buret-molecule-picture img\.buret-xyzrender-card-image\s*\{[^}]*object-fit: contain;/s);

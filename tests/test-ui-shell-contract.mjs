...
assert.match(previewViewer, /function duplicateSceneTreeRepresentation\(ref\)/);
assert.match(previewViewer, /type: params\.type\?\.name/);
assert.match(previewViewer, /color: params\.colorTheme\?\.name/);
assert.match(previewViewer, /size: params\.sizeTheme\?\.name/);
assert.match(previewViewer, /function moleculeMenuRepresentationMode\(submenu, originalRef, renderEditor\)/);
assert.match(previewViewer, /trigger\.addEventListener\('click', event => \{\s*event\.preventDefault\(\);\s*event\.stopPropagation\(\);\s*open\(true\);/);
assert.match(previewViewer, /item\.setAttribute\('role', 'menuitemradio'\)/);
assert.match(previewViewer, /\['current', 'Apply to current'\][\s\S]*\['add', 'Add another'\]/);
assert.match(previewViewer, /await duplicateSceneTreeRepresentation\(originalRef\)/);
assert.match(previewViewer, /sceneTreeRepresentationMenu\(editor, activeViewer, activeNode, activeTarget\)/);
...
assert.match(previewRuntimeCss, /\.buret-representation-mode-radio \.buret-representation-mode-check \{[\s\S]*opacity: 0;/);
assert.match(previewRuntimeCss, /\.buret-representation-mode-radio\[data-checked="true"\] \.buret-representation-mode-check \{[\s\S]*opacity: 1;/);

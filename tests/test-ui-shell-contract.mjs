...
assert.match(previewViewer, /function sdfMoleculesToPdbTrajectory\(molecules, label\)/);
...
assert.match(previewViewer, /function molstarResidueExpression\(residue\)/);
assert.match(previewViewer, /equals\('label_seq_id', seqId\)/);
assert.match(previewViewer, /equals\('auth_comp_id', compId\)/);
assert.match(previewViewer, /equals\('label_asym_id', chainId\)/);
assert.match(previewViewer, /function setMolstarComponentsVisibility\(viewer, components, visible\)/);
...
assert.match(previewViewer, /const nativeTrajectoryPdb = nativeTrajectoryPoseCount > 1/);
assert.match(previewViewer, /format: 'pdb',\s*label: 'Ligand poses'/);
assert.match(previewViewer, /nativeTrajectoryControls: Boolean\(nativeTrajectoryPdb\)/);
...
assert.match(previewViewer, /tryCreateMolstarExpressionComponent\(/);
assert.match(previewViewer, /state\.poseComponents\[index\] = component/);
assert.match(previewViewer, /dockingSdfPoseStateHasPoseCache\(state, singlePdbs\.length\)/);

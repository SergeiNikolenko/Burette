...
assert.match(structureInfoPanel, /option\.value !== "match" && option\.value !== "cartoon" && option\.value !== "spacefill"/);
...
assert.match(structureInfoPanel, /const hasStructureSceneAllMode = document\.dockingRequest\?\.poseMode === "all"/);
assert.match(structureInfoPanel, /const sceneStyle = document\.dockingRequest\?\.sceneMode \? normalizeStructureSceneStyle\(style\) : style/);
assert.match(structureInfoPanel, /if \(sceneStyle !== style\) writeSdfContextStylePreference\(document, sceneStyle\)/);
...
assert.match(structureInfoPanel, /title="All structures"/);
assert.match(structureInfoPanel, /detail="All-together view"/);
assert.match(structureInfoPanel, /ariaLabel="All structures style"/);
assert.match(structureInfoPanel, /opacityLabel="All structures opacity"/);
...
assert.match(structureInfoPanel, /STRUCTURE_SCENE_STYLE_OPTIONS\.some\(\(option\) => option\.value === value\) \? value : "line"/);
...
assert.match(structureInfoPanel, /SDF_CONTEXT_OPACITY_DEFAULT = 0\.12/);
...
assert.match(browserDevDocuments, /const GRID_ASSET_VERSION = "grid-ui-v137"/);
...
assert.match(previewViewer, /keepDefaultMolstarStyle: true/);
...
assert.match(previewViewer, /const contextStructures = await loadSdfCollectionPdbLayer\(viewer, prepared\.data,/);
assert.match(previewViewer, /const contextStyle = readSdfCollectionContextStyle\(activeConfig\)/);
assert.match(previewViewer, /const contextOpacity = readSdfCollectionContextOpacity\(activeConfig\)/);
...
assert.match(previewViewer, /if \(normalized === 'illustrative' \|\| normalized === 'cartoon' \|\| normalized === 'polymer-ligand'\) \{/);
assert.match(previewViewer, /sdfCollectionCartoonRepresentation\(alpha\)/);
assert.match(previewViewer, /sdfCollectionLigandRepresentationForStyle\(normalized, alpha\)/);
...
assert.doesNotMatch(previewViewer, /const color = ghost \? 'uniform' : 'element-symbol'/);
assert.doesNotMatch(previewViewer, /colorParams = ghost \? \{ value: 0x6f7886 \} : undefined/);
assert.match(previewViewer, /const themed = \(representation, color = 'chain-id'\) =>/);
...
assert.match(gridUi, /<ControlTooltip label="Render cards with external xyzrender" \/>/);
assert.match(gridUi, /<ControlTooltip label="Open selected molecules in Molstar" \/>/);
assert.match(gridUi, /data-buret-grid-generate-3d/);
assert.match(gridUi, /Generate 3D/);
assert.match(gridUi, /<ControlTooltip label="Generate 3D conformers for selected molecules" \/>/);

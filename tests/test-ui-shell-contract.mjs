...
assert.match(sidebar, /filterSidebarProjects/);
...
assert.match(sidebar, /Search projects and structures/);
assert.match(sidebar, /ProjectGroup/);
assert.match(sidebar, /project-group-row/);
assert.match(sidebar, /project-children/);
assert.match(sidebar, /project-source-badge/);
assert.match(sidebar, /Open active project folder/);
...
assert.match(styles, /\.sidebar-shell \{[^}]*transition: width 180ms cubic-bezier\(0\.22, 1, 0\.36, 1\)/s);
...
assert.match(sidebar, /Add project folder\.\.\./);
assert.match(sidebar, /Open active project folder/);
...
assert.doesNotMatch(previewRuntimeCss, /\.buret-corner-button \{/);
assert.match(previewRuntimeCss, /body\.burette-quicklook-host \{\s*--buret-toolbar-safe-top: 56px;/s);
assert.doesNotMatch(previewRuntimeCss, /body\.burette-quicklook-host \.buret-corner-button \{/);
...
assert.match(previewViewer, /const mainRect = visibleRect\('\.msp-plugin \.msp-layout-main'\);/);
...
assert.match(previewViewer, /dockToolbar\(toolbar\);\s*fitToolbarToViewport\(toolbar\);\s*const top = toolbarSafeTop\(\);\s*const mainRect = visibleRect\('\.msp-plugin \.msp-layout-main'\);\s*const width = toolbar\.offsetWidth \|\| toolbar\.getBoundingClientRect\(\)\.width \|\| 320;\s*const rightEdge = mainRect \? mainRect\.right : window\.innerWidth;\s*const left = Math\.max\(TOOLBAR_MARGIN, Math\.round\(rightEdge - width - TOOLBAR_MARGIN\)\);\s*toolbar\.dataset\.defaultPosition = '1';\s*toolbar\.style\.left = left \+ 'px';\s*toolbar\.style\.right = 'auto';\s*toolbar\.style\.top = top \+ 'px';\s*updateFloatingLayoutOffsets\(\)/s);

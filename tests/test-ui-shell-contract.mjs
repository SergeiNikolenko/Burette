...
assert.match(browserDevDocuments, /const GRID_ASSET_VERSION = "grid-ui-v58"/);
...
assert.match(gridViewer, /class="buret-filter-fields"/);
assert.match(gridViewer, /aria-label="SMARTS substructure"/);
assert.doesNotMatch(gridViewer, /buret-smarts-control/);
assert.match(gridCss, /\.buret-toolbar-row-main\s*\{[^}]*grid-template-columns: minmax\(260px, 1fr\) minmax\(112px, 148px\) auto;/s);
assert.match(gridCss, /\.buret-filter-control\.has-smarts \.buret-filter-fields\s*\{[^}]*grid-template-columns: minmax\(160px, 1fr\) minmax\(112px, 170px\);/s);
...
assert.doesNotMatch(gridViewer, /cardRenderer:/);
...
assert.doesNotMatch(gridViewer, /XYZRENDER_CARD_CONCURRENCY/);
assert.doesNotMatch(gridViewer, /data-buret-grid-card-renderer/);
assert.doesNotMatch(gridCss, /buret-grid-card-renderer-switch/);
...
assert.doesNotMatch(gridViewer, /async function requestXyzrenderCard/);
assert.doesNotMatch(gridViewer, /function pumpXyzrenderCardQueue/);
...
assert.doesNotMatch(gridViewer, /inputExtension: input\.extension/);
assert.doesNotMatch(gridViewer, /function molblockForRow/);
assert.doesNotMatch(gridViewer, /return \{ extension: 'smi'/);
assert.doesNotMatch(gridViewer, /fetch\(String\(cfg\.xyzrenderEndpoint \|\| '\/__burette\/xyzrender'\)/);

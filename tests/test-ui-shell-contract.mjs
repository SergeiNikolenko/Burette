...
assert.match(viewer, /inputExtension: inputDataBase64 && inputExtension \? inputExtension : undefined/);
...
assert.match(agentPreviewViewer, /function xyzrenderOrientationPayload\(options = \{\}\)/);
assert.match(agentPreviewViewer, /postHostMessage\(\{ type: 'setXyzrenderControls', documentId, controls, preset, \.\.\.xyzrenderOrientationPayload\(options\) \}\)/);
...
for (const runtimeSource of [viewer, agentPreviewViewer]) {
  ...
  assert.match(runtimeSource, /filter\.setAttribute\('x', '-80%'\)/);
  assert.match(runtimeSource, /filter\.setAttribute\('width', '260%'\)/);
  ...
  assert.match(runtimeSource, /createSvgFilterElement\('feDropShadow', \{\n      in: 'SourceGraphic',\n      dx: '0',\n      dy: '0',\n      stdDeviation: '1\.35'/);
  assert.match(runtimeSource, /applyXyzrenderSelectionEffect\(element\);\n      \}\);/);
...
assert.match(commandPalette, /Dialog\.Root/);
...
assert.match(commandPalette, /<Dialog\.Portal container=\{portalContainer\}>/);

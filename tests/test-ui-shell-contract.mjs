...
const applyMolstarLassoPicksBody = viewer.match(/function applyMolstarLassoPicks\(picks, additive\) \{([\s\S]*?)\n  \}/)?.[1] || "";
assert.match(applyMolstarLassoPicksBody, /molstarContextNormalizeLoci\(pick\?\.loci, 'atom'\)/);
assert.match(applyMolstarLassoPicksBody, /selects\.select\(\{ loci \}, false\)/);
assert.match(applyMolstarLassoPicksBody, /selection\.fromLoci\('add', loci, false\)/);
...
assert.match(sidebarSurface, /<RenameIcon \/>/);
assert.match(sidebarSurface, /function RenameIcon\(\)/);
assert.match(sidebarSurface, /onPointerDown=\{\(event\) => \{\s*event\.stopPropagation\(\);\s*startRename\(\);\s*\}\}/);
...
assert.match(previewViewer, /function dockingSdfPoseStateHasPoseCache\(state, poseCount\)/);
...
assert.doesNotMatch(previewViewer, /const nativeTrajectoryPdb = nativeTrajectoryPoseCount > 1/);
assert.doesNotMatch(previewViewer, /format: 'pdb',\s*label: 'Ligand poses'/);
assert.doesNotMatch(previewViewer, /nativeTrajectoryControls: Boolean\(nativeTrajectoryPdb\)/);
assert.match(previewViewer, /nativeTrajectoryControls: false,\s*ligandLabel: 'Ligand poses'/);
assert.match(previewViewer, /collectionPdbData: nativeTrajectoryCollection\?\.data \|\| null/);
assert.match(previewViewer, /FAST_VISIBILITY_LOOP_MAX_FPS = 30/);
assert.match(previewViewer, /supportsFastVisibilityLoop\(prepared\)/);
assert.match(previewViewer, /function minimumTrajectoryLoopFps\(prepared\)/);
assert.match(previewViewer, /supportsFastVisibilityLoop\(prepared\) \? DEFAULT_TRAJECTORY_LOOP_FPS : 0\.1/);
assert.match(previewViewer, /speed\.min = formatTrajectoryFps\(minimumTrajectoryLoopFps\(prepared\)\)/);
assert.doesNotMatch(previewViewer, /tryCreateMolstarExpressionComponent\(/);
assert.doesNotMatch(previewViewer, /state\.poseComponents\[index\] = component/);
assert.match(previewViewer, /const poseEntries = Array\.isArray\(prepared\.poses\) \? prepared\.poses : \[\]/);
assert.match(previewViewer, /dockingSdfPoseStateHasPoseCache\(state, poseCount\)/);
assert.match(previewViewer, /loadMolstarEntryWithStructureRefs\(viewer, entry, \{ representationPreset: 'empty' \}\)/);
assert.match(previewViewer, /const sdfOverlayPoseCount = Math\.max\(/);
...
assert.match(previewViewer, /const FAST_VISIBILITY_LOOP_MAX_FPS = 30/);
assert.match(previewViewer, /if \(prepared\?\.nativeTrajectoryControls\) return Math\.round\(1000 \/ NATIVE_TRAJECTORY_LOOP_MAX_FPS\)/);
assert.match(previewViewer, /if \(supportsFastVisibilityLoop\(prepared\)\) return Math\.round\(1000 \/ FAST_VISIBILITY_LOOP_MAX_FPS\)/);
assert.match(previewViewer, /return 300/);
assert.match(previewViewer, /function minimumTrajectoryLoopTimerDelay\(prepared\)/);
assert.match(previewViewer, /return supportsFastVisibilityLoop\(prepared\) \? 8 : 60/);
...
assert.match(previewViewer, /async function setNativeTrajectoryPoseDirect\(index, poseCount, options = \{\}\)/);
assert.match(previewViewer, /plugin\.state\.updateTransform\(/);
assert.match(previewViewer, /\{ \.\.\.transform\.params, modelIndex: target \}/);
assert.match(previewViewer, /async function setNativeTrajectoryPose\(index, poseCount, options = \{\}\)/);
assert.match(previewViewer, /if \(await setNativeTrajectoryPoseDirect\(target, poseCount, options\)\) return true;/);
...
assert.match(previewViewer, /const switched = await setNativeTrajectoryPose\(nextIndex, prepared\.poseCount, \{ skipPaint: options\.loopStep === true \}\)/);
assert.match(previewViewer, /activePose = options\.loopStep === true \? nextIndex : readNativeTrajectoryPosition\(prepared\.poseCount\)\?\.index \?\? nextIndex/);

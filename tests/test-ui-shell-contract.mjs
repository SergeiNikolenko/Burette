...
assert.match(previewViewer, /if \(all\) mainRow\.append\(all\);\s*animationRow\.append\(speed, loop\);\s*if \(smoothAvailable\) animationRow\.append\(smooth\);\s*animationRow\.append\(slider\);/);
...
assert.match(previewViewer, /animation\.setAttribute\('aria-label', collapsed \? 'Show playback controls' : 'Select Molstar animation'\)/);

...
assert.match(styles, /button, select, input, textarea, \.tab, \.new-tab, \.chrome-button, \.tab-history-button, \.sidebar-search-row, \.project-group-row, \.project, \.splitter \{[^}]*app-region: no-drag;[^}]*-webkit-app-region: no-drag;[^}]*\}/s);
...
assert.match(sidebarProjects, /function compareProjectItems\(left: SidebarProjectItem, right: SidebarProjectItem\) \{\s*return left\.relativePath\.localeCompare\(right\.relativePath\);\s*\}/);
...
assert.match(settingsPanel, /const defaultRendererModeOptions: Array<ViewerPreferences\["rendererMode"\]> = \["auto", "xyz-fast", "molstar", "xyzrender-external"\]/);
...
assert.match(commandPalette, /renderer-xyz-fast/);
...
assert.match(app, /void openDocuments\(\[targetDocument\.path\], undefined, \{ rendererMode: "molstar" \}\)/);
assert.match(app, /body\?\.type === "openSdfGridDocument"/);
assert.match(app, /pushStatus\("Opening SDF grid\.\.\."\)/);
assert.match(app, /void openDocuments\(\[targetDocument\.path\], undefined, \{ rendererMode: "auto" \}\)/);
...
assert.match(browserDevDocuments, /const GRID_ASSET_VERSION = "grid-ui-v35"/);
assert.match(browserDevDocuments, /const VIEWER_ASSET_VERSION = "viewer-ui-v9"/);
...
assert.match(browserDevDocuments, /const explicitSdfViewer = isSdfExtension\(extension\)\s*&& \(requestedMode === "molstar" \|\| \(requestedMode === "xyzrender-external" && Boolean\(reloadOptions\)\)\);/);
...
assert.match(browserDevDocuments, /xyzrenderAvailable \? defaultRendererModeForDocument\(extension, requestedMode, reloadOptions\) : "molstar"/);
...
assert.match(previewViewer, /function prepareSdfPoseStructure\(records, label, config\)/);
...
assert.match(gridCss, /\.buret-molecule-error \{[^}]*font-size: 11px;/s);
assert.match(gridCss, /\.buret-molecule-error strong \{[^}]*max-height: 7\.2em;[^}]*overflow: hidden;/s);
...
assert.match(gridViewer, /function transformedNodeBounds\(node\)/);
...
assert.match(gridViewer, /data-buret-grid-sdf-poses data-buret-grid-docking/);
...
assert.match(previewViewer, /config\.sdfPosePager === true && config\.sdfGrid !== false/);
assert.match(previewViewer, /sdfGridButton\.addEventListener\('click', requestSdfGridDocument\)/);
assert.match(previewViewer, /postHostMessage\(\{ type: 'openSdfGridDocument' \}\)/);

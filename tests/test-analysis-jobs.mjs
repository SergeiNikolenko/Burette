...
for (const scenario of ['success', 'cancel', 'cancel-race', 'failure']) {
  ...
  new Function('require','exports',code)(name => {
    ...
    if (name === '@tauri-apps/api/core') return { Channel: class {}, invoke: async (command, payload) => {
      ...
      return {reportPath:'/final.json',backend:'nativeMetal'};
    } };
    throw new Error(name);
  }, module.exports);
  const promise = module.exports.runAnalysisWorkflow('compute_align_grid_poses', {documentId:'test',sourceIndexes:[0,1]}, 'ensemble');
  if (scenario === 'success') { await promise; assert.equal(jobs.at(-1).reportPath,'/final.json'); }
  else {
  ...
  assert.equal(jobs.at(-1).status, scenario === 'success' ? 'success' : scenario.startsWith('cancel') ? 'cancelled' : 'failed');

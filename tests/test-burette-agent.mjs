...
assert.equal(ligandViaLabelAlias.ok, false, 'Explicit author identifiers must not match label identifiers');
...
const wrongAuthorRange = await context.window.BuretteAgent.run({ command: 'selectResidues', args: { selector: { auth_asym_id: 'X', beg_auth_seq_id: 1, end_auth_seq_id: 2 }, applyToViewer: false } });
assert.equal(wrongAuthorRange.ok, false);
const constrainedAll = await context.window.BuretteAgent.run({ command: 'select', args: { selector: { kind: 'all', auth_seq_id: 300 }, applyToViewer: false } });
assert.equal(constrainedAll.result.counts.atoms, 2);
const wrongModelAll = await context.window.BuretteAgent.run({ command: 'select', args: { selector: { kind: 'all', modelIndex: 99 }, applyToViewer: false } });
assert.equal(wrongModelAll.ok, false);
const secondCell = { transform: { ref: 's1' }, obj: { data: fakeStructure(), label: 'second.pdb' } };
viewer.plugin.managers.structure.hierarchy.current.structures.push({ cell: secondCell });
const primaryOnly = await context.window.BuretteAgent.run({ command: 'select', args: { selector: { kind: 'all', structure: 'primary' }, applyToViewer: false } });
assert.equal(primaryOnly.result.counts.atoms, 6);
const bothObjects = await context.window.BuretteAgent.run({ command: 'select', args: { selector: { kind: 'all' }, applyToViewer: false } });
assert.equal(bothObjects.result.counts.atoms, 12);
const scopedLigand = await context.window.BuretteAgent.run({ command: 'focusLigand', args: { selector: { structure: 'primary', label_comp_id: 'HEM' } } });
assert.equal(scopedLigand.result.counts.atoms, 2);
const objectSummaries = await context.window.BuretteAgent.run({ command: 'summary', args: { includeLigands: true } });
assert.equal(objectSummaries.result.structures[1].ligands[0].structureIndex, 1);
viewer.plugin.managers.structure.hierarchy.current.structures.pop();
...
context.window.BuretteAgent.notifyStructureLoaded({ config: { documentId: 'native-a' } });
const nativeSelection = await context.window.BuretteAgent.run({ command: 'selectResidues', args: { selector: { auth_asym_id: 'A' } } });
assert.equal(nativeSelection.ok, true);
context.window.BuretteAgent.notifyStructureLoaded({ config: { documentId: 'native-b' } });
const staleSelection = await context.window.BuretteAgent.run({ command: 'focusSelection', args: {} });
assert.equal(staleSelection.ok, false);
assert.match(staleSelection.error.message, /No previous selection/u);

...
assert.match(viewer, /function pdbTrajectoryTimesPs\(data\)/);
assert.match(viewer, /function trajectoryPoseLabel\(prepared, controlLabel, activePose\)/);
assert.match(viewer, /Time \$\{timeNs\} ns - \$\{indexText\}/);
assert.match(viewer, /label\.textContent = trajectoryPoseLabel\(prepared, controlLabel, activePose\)/);
...
assert.match(viewer, /const NATIVE_TRAJECTORY_LOOP_SKIP_FPS_THRESHOLD = 25/);
assert.match(viewer, /const NATIVE_TRAJECTORY_LOOP_MAX_FPS = 100/);
assert.match(viewer, /prepared\?\.nativeTrajectoryControls \? Math\.round\(1000 \/ NATIVE_TRAJECTORY_LOOP_MAX_FPS\) : 300/);
...
assert.match(viewer, /const frameOffset = Math\.floor\(elapsed \/ delay\)/);
assert.match(viewer, /return \(loopStartPose \+ frameOffset\) % prepared\.poseCount/);
...
assert.match(viewer, /granularity: action\.granularity \|\| action\.args\?\.granularity/);
assert.match(viewer, /mode: action\.mode \|\| action\.args\?\.mode/);
assert.match(viewer, /if \(type === 'hide_components'\)/);
assert.match(viewer, /if \(type === 'show_components'\)/);
assert.match(viewer, /if \(type === 'clear_selection'\)/);
assert.match(viewer, /function clearMolstarSelection\(\)/);
assert.match(viewer, /lociSelects\?\.deselectAll\?\.\(\)/);
assert.match(viewer, /structure\?\.selection\?\.clear\?\.\(\)/);
assert.match(viewer, /structure\?\.focus\?\.clear\?\.\(\)/);
assert.match(viewer, /async function hideMolstarComponents\(action = \{\}\)/);
assert.match(viewer, /async function showMolstarComponents\(action = \{\}\)/);
assert.match(viewer, /hideComponents: hideMolstarComponents/);
assert.match(viewer, /showComponents: showMolstarComponents/);

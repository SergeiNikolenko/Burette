...
function transformText(text, namespace) {
  ...
  return text
    .replaceAll('<string>burette</string>', `<string>burette-${namespace.slug}</string>`)
...
function shouldPatchFile(path) {
  // This helper must retain canonical identifiers when shipped inside a dev bundle.
  if (path.endsWith('/dev-namespace.mjs')) return false;

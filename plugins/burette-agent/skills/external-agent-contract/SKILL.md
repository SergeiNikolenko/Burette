...
For local molecular work in the native Codex side pane, first use
`burette.open_viewer`, then `burette.observe_inline_viewer` and
`burette.control_inline_viewer` with its `sessionId`. No Browser URL is needed.
This native viewer supports selection/focus, styles/colors, camera rotation,
spin/rock and procedural wiggle, shared file tabs, collections, Ketcher,
docking and packaged Stories. See [open-workspace](../open-workspace/SKILL.md)
for `view` options and native controls. The `workspaceSessionId` facade below

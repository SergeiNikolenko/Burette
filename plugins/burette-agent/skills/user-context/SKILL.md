...
Run the read-only preflight script for setup diagnosis or Browser/desktop and
artifact workflows. A known local PDB/mmCIF path with `burette.open_viewer`
available uses the direct native route in `index`; it needs no separate preflight:

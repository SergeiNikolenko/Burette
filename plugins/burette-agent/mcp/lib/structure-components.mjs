...
export async function extractStructureComponentFile({
  ...
  const outputPath = await writeDerivedFile(outputTitle, contents);
  return {
    outputPath,
    sourcePath,
...
export async function editStructureFragmentFile({
  ...
  const outputPath = await writeDerivedFile(outputTitle, `${[...header, ...outputLines].join("\n").replace(/\n*$/u, "")}\n`);
  return {
    operation,
    outputPath,
    sourcePath,
...
async function writeDerivedFile(title, contents) {
  // One private directory per operation prevents collisions across concurrent
  // tasks while keeping the requested display filename intact.
  const directory = await mkdtemp(path.join(tmpdir(), "burette-agent-component-"));
  const outputPath = path.join(directory, `${title}.pdb`);
  try {

...
export function registerMolecularWorkspace(server) {
  ...
  registerAppTool(
    ...
    async input => {
      ...
      args.push(input.file, ...(input.additionalFiles || []));
      if (input.scene) args.push("--scene", input.scene);
...
function publicContractResult(tool, {
  ...
  return {
    content: toolText(completionState === "awaiting_browser"
      ? `${tool} started. [Open Burette workspace](${session?.url || result?.url}). Show this URL in the Codex right-side Browser; do not create an additional inline viewer. Then call burette.observe_workspace. Do not claim that the structure is visible until ready is true and the central canvas is visually verified.`
      : effectiveOk
        ? `${tool} completed.`

...
| Viewer bridge | Partial | Grid-origin file/runtime/control messages, viewer-origin export/runtime/runtime-file/state/renderer/Ketcher/host messages, shared viewer/grid xyzrender sheet rendering, SDF open messages, and docking pose-change messages now delegate to dedicated hooks, but the `window.message` listener and typed dispatch still remain in `App.tsx`. |

...
- browser-dev startup folder/workspace query derivation now lives in
  `apps/desktop/src/hooks/use-app-browser-dev-startup.ts`;
...
- dock panel tab toggle action assembly now lives in
  `apps/desktop/src/hooks/use-app-dock-actions.ts`;
...
- Quick Look browser-dev document opening and spectrum/text routing now live in
  `apps/desktop/src/hooks/use-app-quick-look-document-open.ts`;
...
- viewer bridge source parsing, document-id extraction, known-source gating, and
  dispatch short-circuit order now live in
  `apps/desktop/src/lib/viewer-bridge-messages.ts`, covered by
  `tests/test-viewer-bridge-message-contract.mjs`;
- viewer/grid message handler composition now lives in
  `apps/desktop/src/hooks/use-app-viewer-bridge-controller.ts`, so `App.tsx`
  wires one bridge controller instead of constructing each message handler and
  passing them into the listener directly;
- viewer runtime refs for pending reloads, Molstar in-place replacement,
  xyzrender orientation, and preference-refresh suppression now live in
  `apps/desktop/src/hooks/use-app-viewer-runtime-refs.ts`;
...
- open-drop molecule collection merge fallback now lives in
  `apps/desktop/src/hooks/use-app-open-drop-merge-collections.ts`;
- open/drop event registration, agent-session wiring, open-from-clipboard, and
  drop controller composition now live in
  `apps/desktop/src/hooks/use-app-open-drop-controller.ts`, while `App.tsx`
  receives only the layout-facing drop handlers and `openClipboard` action;
...
- host runtime operations for grid runtime shutdown, active preview SVG/PNG
  export, and grid perf metric persistence now live in
  `apps/desktop/src/hooks/use-app-host-runtime-operations.ts`;
- viewer preference sync and active preview refresh side effects now live in
  `apps/desktop/src/hooks/use-app-preference-effects.ts`;
- settings/back navigation callbacks now live in
  `apps/desktop/src/hooks/use-app-shell-navigation-actions.ts`;
- active viewer reload and xyzrender control reload actions now live in
  `apps/desktop/src/hooks/use-app-viewer-reload-actions.ts`;
...
| App shell extraction | Partial | Several app hooks, pure chemistry libs, file-routing helpers, shared file-export/clipboard/preview-image helpers, host runtime operation helpers, preference effects, shell navigation callbacks, viewer reload actions, browser-dev bundle helpers, content spectrum detection, the core file-open hook, file picker/recent-open actions, the dock payload-open hook, dock panel tab toggle actions, browser-dev startup URL helpers/effects, pure Ketcher workflow helpers, Ketcher action callbacks, docking/collection action callbacks, grid append/xyzrender sheet callbacks, grid save/export/runtime/control/conformer message handling, viewer export/runtime/runtime-file/state/renderer/Ketcher/host/conformer/Molstar-context message handling, top-level viewer/grid dispatch, viewer runtime refs, xTB/CREST status/settings/cancel state, CREST/PRISM workflow runners, 3D conformer generation/Molstar replacement sender, Molstar text-selection/action senders, Molstar xTB context request handling, xTB workflow runners, shared xyzrender sheet message handling, SDF open message handling, docking pose-change message handling, FEP setup/network callbacks, drop action callbacks, open-drop collection merge fallback, open/drop/clipboard controller composition, workspace/project-folder callbacks, agent-session tab action assembly, ShellViewState assembly, and ShellActions assembly are extracted. Remaining app-shell work is smaller composition cleanup plus runtime hardening boundaries. |
| Opening workflow | Partial | `openDocuments`, `openPaths`, text/spectrum opening, path classification, pasted-structure opening, file picker/recent-open actions, dock payload opening, open/drop event composition, open-from-clipboard wiring, Quick Look browser-dev document opening, browser-dev startup URL parsing, and browser-dev startup orchestration effects are in dedicated modules/hooks. |
...
Viewer bridge extraction slices must include:

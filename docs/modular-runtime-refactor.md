...
| Viewer bridge | Partial | Grid-origin file/runtime/control messages, viewer-origin export/runtime messages, and shared viewer/grid xyzrender sheet rendering now delegate to dedicated hooks, but the `window.message` listener and typed dispatch still remain in `App.tsx`. |

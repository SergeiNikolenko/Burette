...
| App shell extraction | Partial | Several app hooks, pure chemistry libs, file-routing helpers, shared file-export helpers, the core file-open hook, the dock payload-open hook, browser-dev startup URL helpers, and pure Ketcher workflow helpers are extracted, but browser-dev startup effects, Ketcher action wiring, docking, collections, xTB/conformer controllers, and message handling still live in `App.tsx`. |

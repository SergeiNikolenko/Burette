...
JSONL содержит ровно две готовые структуры с преобразованиями. Попытка заменить read-only CIF через системный Save/Replace вернула ошибку
«is read-only» и сохранила Unsaved changes и кнопку Save. Закрытие изменённой
вкладки показало предупреждение Unsaved Changes; исходный файл не изменён. TypeScript, Clippy и focused tests
...
приложение не заменялось; изменения подготовлены в рабочей копии и dev-сборке.

...
реальное обновление, Quick Look, iPhone и научная точность всех методов в этом
проходе не проверялись. Первоначальные находки ниже описывают состояние базы аудита.
...
кеша разобранных поз и XYZ, части обновлений оформления, уступки UI при Align,
...
**Работа не завершена целиком.** Остаются ранний ID/прогресс и остановка между
молекулами для alignment/semiempirical, дальнейшая работа с очередью оформления и
крупными Align, нативная проверка Save/Cancel/ошибки записи, пакетного открытия и
отмены расчётов. Кеши ограничены размером исходного текста, это не измеренный предел
RSS; Undo сохраняет хотя бы одну запись даже сверх бюджета. Проценты ускорения не
измерены. Установленное приложение не обновлялось.

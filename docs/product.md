...
| Conformer generation | Eight-variant IDs, chunk-invariant seeds, adaptive `N x K` planning, strict pack/stage contracts, typed durable snapshot/submission contracts, packaged Metal initialization, and DG distance energy/gradient implemented; coordinator execution/publication, parameter extraction, iterative DG/ETK embedding, and UI remain in progress |

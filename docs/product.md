...
| Conformer generation | Eight-variant IDs, chunk-invariant seeds, adaptive `N x K` planning, strict pack/stage contracts, packaged Metal initialization, and DG distance energy/gradient implemented; parameter extraction, iterative DG/ETK embedding, durable snapshots/publication, and UI remain in progress |

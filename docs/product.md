...
| Conformer generation | Native eight-variant IDs, deterministic chunk-invariant seed, adaptive `N x K` planner, and strict shared-constraint EnginePack ABI implemented; parameter extraction, DG/ETK kernels, durable ResultPacks, and UI remain in progress |

...
const agentShellDistDir = process.env.BURETTE_AGENT_SHELL_DIST_DIR
  ? resolve(process.env.BURETTE_AGENT_SHELL_DIST_DIR)
  : defaultAgentShellDistDir();
const browserDevGeneratedFilesRoot = process.env.BURETTE_BROWSER_DEV_GENERATED_FILES_ROOT
...
async function open(options) {
  ...
  if (options.rest.length > 1 && !['auto', 'browser-agent-shell'].includes(mode)) {
    fail('INVALID_ARGS', 'Opening multiple files requires browser-agent-shell mode.', 2);
  }
...
async function openAuto(file, options) {
  try {
    ...
    if (!error?.buretteAgentError) throw error;
    if (options.rest.length > 1 || options.scene) fail(error.code, error.message, 1, error.details);
    await openBrowserPreview(file, options, {
...
async function openBrowserPreview(file, options, fallback = null) {
  ...
  const sessionDir = options.sessionDir ? resolve(options.sessionDir) : await mkdtemp(resolve(tmpdir(), 'burette-agent-preview-'));
  ...
  try {
    ...
    if (error?.buretteAgentError) fail(error.code, error.message, 1, error.details);
    throw error;
  }
  child.unref();
  console.log(JSON.stringify({
...
async function openBrowserAgentShell(file, options) {
  ...
  const initialPaths = sceneFiles || options.rest.map(path => resolve(path));
  if (initialPaths.length > SCENE_MAX_FILES) fail('INVALID_ARGS', `A workspace can open at most ${SCENE_MAX_FILES} files at once.`, 2);
  const initialFile = initialPaths[0];
  ...
  if (sceneFiles) {
    ...
    url.searchParams.set('devFiles', initialPaths.join('\n'));
  }
  ...
  const env = {
    ...process.env,
    BURETTE_DEV_DEFAULT_FILES: initialPaths.join('\n'),
    BURETTE_DEV_FS_ALLOW: browserDevFsAllowRoots(initialFile, initialPaths).join(delimiter),
    BURETTE_BROWSER_DEV_GENERATED_FILES_ROOT: browserDevGeneratedFilesRoot,
    BURETTE_AGENT_SHELL_SESSION_DIR: sessionDir,
    VITE_BURETTE_AGENT_SHELL: '1',
    VITE_BURETTE_BUILD_IDENTIFIER: 'browser-agent-shell',
    VITE_BURETTE_DEV_INSTANCE: 'agent',
  };
  const logPath = resolve(sessionDir, 'server.log');
  if (hasPrebuiltAgentShell() && process.env.BURETTE_AGENT_SHELL_FORCE_VP !== '1') {
  ...
  try {
    ...
    if (options.recover && error?.buretteAgentError) throw error;
    if (error?.buretteAgentError) fail(error.code, error.message, 1, error.details);
    throw error;
  }
  child.unref();
  console.log(JSON.stringify({
    ...
    result: {
      ...
      observe: `node scripts/burette-agent.mjs observe --session-dir ${JSON.stringify(sessionDir)}`,
      act: `node scripts/burette-agent.mjs act --session-dir ${JSON.stringify(sessionDir)} '<json-action>'`,
    },
  }, null, 2));
}
...
async function openPrebuiltBrowserAgentShell({ initialPaths, sessionDir, host, port, url, logPath, options }) {
  const logHandle = await openFile(logPath, 'a');
  let childExit = null;
  const child = spawn(process.execPath, [
    ...
    ...browserDevFsAllowRoots(initialPaths[0], initialPaths).flatMap((root) => ['--allow', root]),
    '--host', host,
    '--port', String(port),
  ], {
    cwd: repoRoot,
...
function browserDevFsAllowRoots(initialFile, extraFiles = []) {
  const explicitRoots = (process.env.BURETTE_DEV_FS_ALLOW ?? "").split(delimiter).filter(Boolean);
...
async function openDesktopApp(file, options) {
  const sessionDir = options.sessionDir ? resolve(options.sessionDir) : await mkdtemp(resolve(tmpdir(), 'burette-agent-'));
  await mkdir(sessionDir, { recursive: true });
  const token = randomUUID();
  await writeJsonFile(resolve(sessionDir, 'session.json'), {
    apiVersion,
...
function desktopOpenArgs(app, sessionDir, file) {
  const agentArgs = ['--args', '--burette-agent-session', sessionDir, file];
...
function requireLocalAgentUrl(value, code, details = null) {
  ...
  try {
    ...
    fail(code, `Burette agent URL is invalid: ${String(value)}.`, 1, details);
  }
  const port = Number.parseInt(url.port, 10);
  const loopbackHosts = new Set(['127.0.0.1', 'localhost', '[::1]', '::1']);
  if (url.protocol !== 'http:' || !loopbackHosts.has(url.hostname.toLowerCase()) || !Number.isInteger(port) || port <= 0 || port > 65535) {
    fail(code, 'Burette agent URLs must use http://127.0.0.1:<port>, http://localhost:<port>, or http://[::1]:<port>.', 1, {
      ...details,
      url: url.toString(),
    });
  }
...
function requireLocalHost(value) {
  ...
  fail('INVALID_HOST', '--host must be 127.0.0.1, localhost, or ::1 for a local Burette agent workspace.', 2, { host });
}

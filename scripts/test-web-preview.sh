...
const appDelegate = fs.readFileSync('App/Platform/PlatformBridge.swift', 'utf8');
const updater = fs.readFileSync('App/BurreteUpdater.swift', 'utf8');
const appViewer = fs.readFileSync('App/Platform/MoleculeViewerWindowController.swift', 'utf8');
const appViewerScene = fs.readFileSync('App/MoleculeViewerScene.swift', 'utf8');
const quickLookViewer = fs.readFileSync('PreviewExtension/Platform/PreviewViewController.swift', 'utf8');
...
const viewerRuntimeCSS = fs.readFileSync('PreviewExtension/Web/viewer-runtime.css', 'utf8');
const viewerShell = fs.readFileSync('PreviewExtension/Web/viewer-shell.js', 'utf8');
...
assert(viewerShell.includes('role="toolbar"'), 'shared viewer shell must expose a toolbar role');
...
assert(viewerShell.includes('aria-label="Expand controls"'), 'toolbar handle should default to expanding collapsed controls');
assert(viewerShell.includes('aria-expanded="false"'), 'toolbar handle should expose collapsed state');
assert(index.includes('./viewer-runtime.css'), 'preview HTML must load the shared viewer runtime CSS');
assert(viewerRuntimeCSS.includes('top: var(--buret-toolbar-safe-top); right: 12px; left: auto'), 'toolbar should honor the safe top inset');
assert(viewerShell.includes('data-buret-action="theme"'), 'toolbar should expose a separate theme toggle');
assert(viewerShell.includes('data-buret-action="open-vesta"'), 'toolbar should expose a VESTA handoff button');
assert(viewerRuntimeCSS.includes('--buret-molstar-panel-background'), 'shared viewer CSS must define Mol* theme panel colors');
assert(viewerRuntimeCSS.includes('.msp-viewport-controls-panel'), 'shared viewer CSS must theme Mol* viewport panels');
assert(viewerRuntimeCSS.includes('.msp-viewport-controls-panel .msp-control-group-header > button'), 'shared viewer CSS must style Mol* floating settings panel headers');
assert(viewerRuntimeCSS.includes('.msp-selection-viewport-controls > .msp-flex-row'), 'shared viewer CSS must style Mol* selection viewport toolbar');
assert(viewerRuntimeCSS.includes('--buret-molstar-panel-radius'), 'shared viewer CSS must define a shared Mol* panel radius');
assert(viewerRuntimeCSS.includes('.msp-hover-box-wrapper .msp-hover-box-body'), 'shared viewer CSS must round Mol* hover panels');
assert(viewerRuntimeCSS.includes('.msp-action-menu-options'), 'shared viewer CSS must round Mol* action menu panels');
assert(viewerRuntimeCSS.includes('.msp-snapshot-description-wrapper *'), 'shared viewer CSS must theme Mol* snapshot description text');
assert(viewerRuntimeCSS.includes('.msp-sequence-select > select'), 'shared viewer CSS must theme Mol* sequence selectors');
assert(viewerRuntimeCSS.includes('top: 64px !important'), 'Mol* viewport controls should clear the toolbar row');
...
assert(updater.includes('burreteUpdateStateDidChange'), 'updater must publish update state changes for in-app notifications');
...
assert(appViewerScene.includes('.regularMaterial'), 'app viewer transparency must use native SwiftUI material instead of a fully clear custom window');
...
assert(buildScript.includes('PreviewExtension/Web/viewer-runtime.css'), 'build script must require shared viewer runtime CSS');
assert(buildScript.includes('PreviewExtension/Web/viewer-shell.js'), 'build script must require and syntax-check shared viewer shell');
...
assert(releaseScript.includes('PreviewExtension/Web/viewer-runtime.css'), 'release script must require shared viewer runtime CSS');
assert(releaseScript.includes('PreviewExtension/Web/viewer-shell.js'), 'release script must require and syntax-check shared viewer shell');
assert(xcodeProject.includes('PreviewExtension/Web/burette-agent.js'), 'Xcode validation phase must track burette-agent.js');
assert(xcodeProject.includes('PreviewExtension/Web/viewer-runtime.css'), 'Xcode validation phase must track shared viewer runtime CSS');
assert(xcodeProject.includes('PreviewExtension/Web/viewer-shell.js'), 'Xcode validation phase must track shared viewer shell');
...
assert(index.includes('./viewer-shell.js'), 'preview HTML must load the shared viewer shell');
assert(viewerShell.includes('buret-renderer-choice'), 'shared viewer shell must use compact renderer choice controls');
assert(viewerShell.includes('aria-label="Expand controls"'), 'shared viewer shell should default the compact toolbar to a collapsed affordance');
assert(viewerRuntimeCSS.includes('#buret-toolbar.collapsed:hover'), 'shared viewer CSS should reveal compact toolbar controls on hover');
assert(!/>L<|>Seq<|>Light<|>VESTA</.test(viewerShell), 'shared viewer shell must not expose legacy text toolbar controls');

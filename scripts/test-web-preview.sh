...
assert(index.includes('top: var(--buret-toolbar-safe-top); left: 12px; right: auto'), 'toolbar should honor the safe top inset');
...
assert(viewer.includes("TOOLBAR_POSITION_VERSION = '4'"), 'toolbar position cache should invalidate pre-safe-area positions');

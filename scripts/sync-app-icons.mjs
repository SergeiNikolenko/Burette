...
const additionalSourceNotice = (snapshot.additionalSources ?? [])
  .map((source) => `${source.package}@${source.version} (${source.license})`)
  .join(', ');
const sourceNotice = additionalSourceNotice
  ? `${snapshot.source.package}@${snapshot.source.version}; additional geometry: ${additionalSourceNotice}`
  : `${snapshot.source.package}@${snapshot.source.version}`;
const banner = `/*! ${sourceNotice}\n${license.trim()}\n*/\n// Generated by scripts/sync-app-icons.mjs. Source: config/icons/apps-sdk.json.\n`;

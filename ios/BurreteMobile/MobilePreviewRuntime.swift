        ...
        <head>
          ...
          <script>
            (function () {
              ...
              window.BurreteMobileControls = window.BurreteMobileControls || {
                pendingLayoutState: null,
                setLayout: function (state) {
                  this.pendingLayoutState = state || {};
                  if (typeof window.BurreteApplyMobileLayoutState === 'function') {
                    window.BurreteApplyMobileLayoutState(this.pendingLayoutState);
                  }
                },
                runAction: function (name) {
                  if (typeof window.BurreteRunMobileControlAction === 'function') {
                    window.BurreteRunMobileControlAction(String(name || ''));
                  }
                },
                runContextMenuAction: function (action, mode) {
                  if (typeof window.BurreteRunMobileContextMenuAction === 'function') {
                    window.BurreteRunMobileContextMenuAction(String(action || ''), String(mode || 'molecule'));
                  }
                }
              };

...
final class PlatformStatusMenuController {
    ...
    func install(target: AppLifecycleBridge) {
        ...
        menu.addItem(NSMenuItem(title: "Copy Log Path", action: #selector(AppLifecycleBridge.copyLogPath), keyEquivalent: ""))
        menu.addItem(NSMenuItem(title: "Clear Preview Cache", action: #selector(AppLifecycleBridge.clearPreviewCacheFromMenu), keyEquivalent: "k"))
        menu.addItem(NSMenuItem(title: "Reset Quick Look", action: #selector(AppLifecycleBridge.resetQuickLook), keyEquivalent: "r"))
...
enum PlatformActions {
    static var logsDirectory: URL { AppLifecycleBridge.logsDirectory }
    static var previewCacheDirectory: URL { AppLifecycleBridge.previewCacheDirectory }
    static var primaryLogURL: URL { AppLifecycleBridge.primaryLogURL }

    static func clearPreviewCache() {
        AppLifecycleBridge.clearPreviewCache()
    }

    static func openLogsDirectory() {
        AppLifecycleBridge.openLogsDirectory()
    }

    static func copyLogPath() {
        NSPasteboard.general.clearContents()
        NSPasteboard.general.setString(primaryLogURL.path, forType: .string)
    }
    ...
    static func registerAsDefaultHandler() -> String {
        BurreteFileAssociations.registerAsDefaultHandler()
    }

    static var defaultHandlerSummary: String {
        BurreteFileAssociations.defaultHandlerSummary
    }
...
struct ViewerRuntimePreferences: Equatable {
